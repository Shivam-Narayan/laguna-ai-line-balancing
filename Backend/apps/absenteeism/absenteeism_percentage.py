...
def calculate_line_percentages_older_version(data, date, selected_line='ALL'):
    ...
    if not date_data.empty:
        ...
        for line_name, line_group in line_groups:
            ...
            line_total_employees = line_group['Total_Employees'].iloc[0] if 'Total_Employees' in line_group.columns else len(line_group)
        ...
        return round(sum(line_percentages) / len(line_percentages), 1) if line_percentages else 0.0
    else:
        ...
        if not similar_data.empty:
            ...
            if not similar_data.empty:
                ...
                for single_date in similar_data['date'].unique():
                    ...
                    if day_percentages:
                        date_percentages.append(sum(day_percentages) / len(day_percentages))
            
            # Take average of all similar dates
            return round(sum(date_percentages) / len(date_percentages), 1) if date_percentages else 0.0
        else:
            return 0.0
