...
def reallocate_work(affected_allocations, emp_fact_df, allocation_date, emp_type=None):
    ...
    for index, row in affected_allocations.iterrows():
        ...
        if available_employee.empty:
            available_employee = get_prioritized_employees(emp_fact_df, line=line, code=code, emp_type="Secondary")
...
def perform_dday_allocation(allocation_date, attendance_df, emp_fact_df, wip_df=None):
    ...
    for idx in final_manning.index:
        ...
        has_shortage = final_manning.loc[idx, 'shortage_flag'] in ["Partially Shortage", "Shortage Unresolved"]
    ...
    for idx in final_manning.index:
        ...
        if pd.isna(emp_id) or emp_id == "":  # If original_emp is empty or NaN, use allocated_emp_id
    ...
    stats = {
        ...
        'successful_reallocations': len(reallocated_manning[reallocated_manning['shortage_flag'] == 'Reallocated']),
        'failed_reallocations': len(reallocated_manning[reallocated_manning['shortage_flag'] != 'Reallocated']),  # Changed to catch all failure types
        'reallocation_by_level': pd.Series(reallocated_manning['reallocation_level']).value_counts().to_dict(),
        'reallocation_by_reason': pd.Series(final_manning.loc[reallocated_manning.index, 'reallocation_reason']).value_counts().to_dict()
    }
...
def adjust_planned_quantities_with_wip(manning_df, wip_df):
    ...
    key_columns = [
        'OC NO', 'ORDER NO', 'BUYER', 'STYLE', 'LINE', 'COLOR',
        'SECTION', 'OPERATION', 'CODE'
    ]

    # Make sure all necessary columns exist in both dataframes
    existing_key_columns = [col for col in key_columns if col in updated_manning.columns and col in wip_df.columns]
        ...
        if len(matching_operations) > 1:
            ...
            if total_planned_qty > 0:
                ...
                for idx in matching_operations.index:
                    ...
                    new_planned_qty = max(0, operation_planned_qty - operation_wip_share)

                    # Update the planned quantity
                    updated_manning.loc[idx, 'PLANNED QTY'] = new_planned_qty

                    # Also update allocated capacity and target capacities based on new planned quantities
                    if 'TARGET@100%' in updated_manning.columns and pd.notna(updated_manning.loc[idx, 'ALLOCATED CAPACITY']):
...
def perform_intraday_allocation(allocation_date, attendance_df, emp_fact_df, previous_manning, wip_df=None, current_time=None):
    """
    ...
    end_of_day_time = pd.Timestamp.combine(allocation_date.date(), pd.Timestamp(END_OF_DAY_THRESHOLD).time()).tz_localize(pytz.timezone('Asia/Kolkata'))
    ...
    initial_run_time = pd.Timestamp.combine(allocation_date.date(), pd.Timestamp(DEFAULT_WORK_START_TIME).time()).tz_localize(pytz.timezone('Asia/Kolkata'))
    work_day_end_time = pd.Timestamp.combine(allocation_date.date(), pd.Timestamp(DEFAULT_WORK_END_TIME).time()).tz_localize(pytz.timezone('Asia/Kolkata'))
    ...
    for emp_id, allocated in employee_allocations.items():
        ...
        if not emp_rows.empty:
            ...
            # Update the capacity
            updated_emp_fact.loc[updated_emp_fact['employee_id'] == emp_id, 'remaining_capacity'] = remaining_capacity
    ...
    if not daily_attendance.empty:
        if 'last_updated' in daily_attendance.columns:
            ...
            daily_attendance = daily_attendance[daily_attendance['last_updated'] >= initial_run_time]
            latest_attendance = daily_attendance.sort_values('last_updated').groupby('employee_id').last()
        else:
            latest_attendance = daily_attendance.groupby('employee_id').first()
    ...
    newly_absent = [emp_id for emp_id in current_absent if emp_id not in previously_absent]

    affected_allocations = identify_affected_allocations(working_manning, newly_absent, handle_shortages=True)
    ...
    for idx in reallocated_manning.index:
        ...
        # Preserve original employee information
        if 'original_emp' not in final_manning.columns or pd.isna(final_manning.loc[idx, 'original_emp']):
            # This is the first reallocation for this row
            final_manning.loc[idx, 'original_emp'] = working_manning.loc[idx, 'allocated_emp_id']
            if 'original_emp_name' in final_manning.columns:
                final_manning.loc[idx, 'original_emp_name'] = working_manning.loc[idx, 'ALLOCATED EMP NAME']
    ...
    if 'attendance_status' in final_manning.columns:
        final_manning['attendance_status'] = final_manning['attendance_status'].astype(str)
        for idx in final_manning.index:
            emp_id = final_manning.loc[idx, 'allocated_emp_id']
            if emp_id in attendance_status_map:
                final_manning.loc[idx, 'attendance_status'] = attendance_status_map[emp_id]
    ...
    # Update run history
    if 'run_history' in final_manning.columns and 'current_run' in final_manning.columns:
        for idx in final_manning.index:
            history = safe_eval_history(final_manning.at[idx, 'run_history'])

            if idx in reallocated_manning.index:
                reason = final_manning.at[idx, 'reallocation_reason'] if 'reallocation_reason' in final_manning.columns else 'unknown'
                history.append({
                    'run_timestamp': run_timestamp,
                    'run_type': 'intraday',
                    'previous_emp': working_manning.at[idx, 'allocated_emp_id'],
    ...
    stats = {
        ...
        'allocations_affected': len(affected_allocations),
        'successful_reallocations': len(reallocated_manning[reallocated_manning['shortage_flag'] == 'Reallocated']),
        'failed_reallocations': len(reallocated_manning[reallocated_manning['shortage_flag'] != 'Reallocated']),
        'reallocation_by_level': pd.Series(reallocated_manning['reallocation_level']).value_counts().to_dict() if 'reallocation_level' in reallocated_manning.columns else {},
        'reallocation_by_reason': pd.Series(final_manning.loc[reallocated_manning.index, 'reallocation_reason']).value_counts().to_dict() if 'reallocation_reason' in final_manning.columns else {},
...
def run_intraday_allocation(base_path='csv_files',
                          date_str=None,
                          specify_time=None,
                          mode=None):
    """
    ...
    if specify_time:
        current_time = pd.Timestamp.combine(
            ...
            pd.Timestamp(specify_time).time()
        )
    else:
        current_time = get_ist_time()
    ...
    try:
        ...
        attendance_df['Attendance Date'] = pd.to_datetime(attendance_df['Attendance Date'], format='%d-%m-%Y', errors='coerce')
        ...
        # Filter employees
        emp_fact_df = emp_fact_df[emp_fact_df["EMPLOYEE ID"].isin(Act_Employees["Emp No"])]
        ...
        end_of_day_time = pd.Timestamp.combine(
            allocation_date.date(),
            pd.Timestamp(END_OF_DAY_THRESHOLD).time()
...
def perform_end_of_day_allocation_new(allocation_date, attendance_df, emp_fact_df, previous_manning, wip_df=None, current_time=None):
    """
    ...
    reset_columns = [
        'original_emp', 'original_emp_name', 'new_emp', 'reallocation_level', 
        're_allocated_employee', 'reallocation_reason'
    ]
    ...
    tomorrow_attendance = attendance_df[attendance_df['attendance_date'] == allocation_date]
    ...
    if not tomorrow_attendance.empty:
        ...
        if 'last_updated' in tomorrow_attendance.columns:
            tomorrow_attendance['last_updated'] = pd.to_datetime(tomorrow_attendance['last_updated'], errors='coerce')
            latest_attendance = tomorrow_attendance.sort_values('last_updated').groupby('employee_id').last()
        else:
            latest_attendance = tomorrow_attendance.groupby('employee_id').first()
        
        # Identify employees who are planned to be absent tomorrow
        planned_absent = latest_attendance[latest_attendance['status'] == 'A'].index.tolist()
        
        # Identify employees who are planned to be present tomorrow
        planned_present = latest_attendance[latest_attendance['status'] == 'P'].index.tolist()
    ...
    updated_emp_fact.loc[updated_emp_fact['employee_id'].isin(planned_absent), 'remaining_capacity'] = 0
    ...
    affected_allocations = identify_affected_allocations(tomorrow_manning, planned_absent, handle_shortages=True)
    ...
    for idx in reallocated_manning.index:
        ...
        elif tomorrow_manning.loc[idx, 'shortage_flag'] in ["Partially Shortage", "Shortage Unresolved"]:
            reallocation_reason = 'shortage_resolution'
        else:
            reallocation_reason = 'other'
        ...
        final_manning.loc[idx, 'original_emp'] = tomorrow_manning.loc[idx, 'allocated_emp_id']
        if 'original_emp_name' in final_manning.columns:
            final_manning.loc[idx, 'original_emp_name'] = tomorrow_manning.loc[idx, 'ALLOCATED EMP NAME']
    ...
    # Calculate statistics
    shortage_rows = tomorrow_manning[tomorrow_manning['shortage_flag'].isin(["Partially Shortage", "Shortage Unresolved"])]
...
def fetch_and_transform_planned_leaves(today, nextDay, attendance_df):
    ...
    employees_on_leave = get_employees_on_leave_tomorrow(today, nextDay, planned_attendance_df)
    ...
    daily_attendance = attendance_df[attendance_df['attendance_date'] == allocation_date_today]
...
def get_employees_on_leave_tomorrow(today, nextDay, planned_attendance_df):
    ...
    planned_attendance_df['From'] = pd.to_datetime(planned_attendance_df['From'], dayfirst=True).dt.date
    planned_attendance_df['To'] = pd.to_datetime(planned_attendance_df['To'], dayfirst=True).dt.date
...
def handle_startofday(allocation_date, attendance_df, emp_fact_df, wip_df, has_wip_data, **kwargs):
    manning, stats, tracking = perform_dday_allocation(
        allocation_date,
        attendance_df,
        emp_fact_df,
...
def handle_intraday(allocation_date, attendance_df, emp_fact_df, wip_df, has_wip_data, current_time, **kwargs):
...
def handle_endofday(allocation_date, attendance_df, emp_fact_df, wip_df, has_wip_data, current_time, current_date, **kwargs):
