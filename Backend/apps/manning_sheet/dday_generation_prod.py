...
class EmployeeCapacityTracker:
    ...
    def calculate_effective_remaining_capacity_vectorized(self, emp_df):
        ...
        result_df["effective_remaining_capacity"] = result_df.apply(calculate_row, axis=1)
    ...
    def allocate_capacity(self, emp_id, allocation):
        ...
        self.emp_fact_df.loc[mask, "remaining_capacity"] = self.emp_fact_df.loc[mask].apply(
            lambda row: max(0, row["average_capacity"] - self.total_used_capacity[emp_id]),
            axis=1
        )
            ...
            if used > 0:
                ...
                if not emp_info.empty:
                    ...
                    summary.append({
                        ...
                        "USED CAPACITY": used,
                        "UTILIZATION PCT": min(100, (used / primary_capacity) * 100) if primary_capacity > 0 else 0
                    })
...
def get_prioritized_employees_new(emp_df, line=None, factory=None, floor=None, code=None, emp_type=None):
    """
    ...
    if emp_type:
        query &= (emp_df["type"] == emp_type)  # This ensures Primary and Secondary are handled separately

    return emp_df[query].sort_values(by=["type", "remaining_capacity"], ascending=[True, False])



def check_floaters_for_allocation(emp_fact_df, code, line=None, factory=None, floor=None):
    """
...
def find_preferred_employees_new(emp_fact_df, code=None, line=None, factory=None, floor=None, current_emp_id=None):
    """
    ...
    # First try same line
    if line:
        same_line_employees = get_prioritized_employees(emp_fact_df, line=line, code=code)
    ...
    # Try same factory
    if factory:
        same_factory_employees = get_prioritized_employees(emp_fact_df, factory=factory, code=code)
    ...
    # Try same floor
    if floor:
        same_floor_employees = get_prioritized_employees(emp_fact_df, floor=floor, code=code)
...
def identify_affected_allocations(planned_manning_df, absent_employees, handle_shortages=True):
    ...
    if handle_shortages:
        shortage_allocations = planned_manning_df[
            planned_manning_df['shortage_flag'].isin(["Partially Shortage", "Shortage Unresolved"])
        ].copy()
...
def balance_by_operation_sequence_enhanced_new(planned_manning_df, emp_fact_df, wip_df=None):
    """
    ...
    for idx, row in balanced_manning.iterrows():
        if pd.notna(row['allocated_capacity']) and row['allocated_capacity'] > 0:
            ...
            if hours_needed < 6 and row['wip_quantity'] > hourly_output * 1:
                ...
                balanced_manning.at[idx, 'excess_capacity'] = excess_hours * (allocated_capacity / 8)
        ...
        if not same_line_donors.empty:
            ...
            if emp_has_skill:
                ...
                if transfer_capacity > 0:
                    ...
                    balanced_manning.at[donor_idx, 'excess_capacity'] -= transfer_capacity
                    ...
                    balanced_manning.at[idx, 'reallocation_level'] = 'critical_shortage_prevention'
    ...
    for line, group in balanced_manning.groupby('line'):
        ...
        for i in range(len(line_ops) - 1):
            ...
            if (current_status == 'EXCESS' and
                ...
                balanced_manning.at[current_op_idx, 'excess_capacity'] > 0):
...
def reallocate_work_enhanced_new(affected_allocations, emp_fact_df, allocation_date, wip_df=None, emp_type=None):
    """
    ...
    for index, row in affected_allocations.iterrows():
        ...
        if available_employee.empty:
            ...
            available_employee = get_prioritized_employees(emp_fact_df, line=line, code=code, emp_type=emp_type)
            allocation_level = 'same_line'

        if available_employee.empty:
            # Try same line with secondary skill
            available_employee = get_prioritized_employees(emp_fact_df, line=line, code=code, emp_type="Secondary" if emp_type == "Primary" else "Primary")
            allocation_level = 'same_line_secondary'

        if available_employee.empty:
            # Try same factory with primary skill
            available_employee = get_prioritized_employees(emp_fact_df, factory=factory, code=code, emp_type=emp_type)
            allocation_level = 'same_factory'

        if available_employee.empty:
            # Try same factory with secondary skill
            available_employee = get_prioritized_employees(emp_fact_df, factory=factory, code=code, emp_type="Secondary" if emp_type == "Primary" else "Primary")
            allocation_level = 'same_factory_secondary'

        if available_employee.empty:
            # Try same floor with primary skill
            available_employee = get_prioritized_employees(emp_fact_df, floor=floor, code=code, emp_type=emp_type)
            allocation_level = 'same_floor'

        if available_employee.empty:
            # Try same floor with secondary skill
            available_employee = get_prioritized_employees(emp_fact_df, floor=floor, code=code, emp_type="Secondary" if emp_type == "Primary" else "Primary")
            allocation_level = 'same_floor_secondary'

        if available_employee.empty:
            # Try anywhere with primary skill
            available_employee = get_prioritized_employees(emp_fact_df, code=code, emp_type=emp_type)
            allocation_level = 'other_location'

        if available_employee.empty:
            # Try anywhere with secondary skill
            available_employee = get_prioritized_employees(emp_fact_df, code=code, emp_type="Secondary" if emp_type == "Primary" else "Primary")
        ...
        if not available_employee.empty:
            ...
            for i in range(len(available_employee)):
                ...
                emp_fact_df.loc[emp_fact_df['employee_id'] == emp_id, 'remaining_capacity'] = new_remaining
                ...
                affected_allocations.at[index, 'allocated_emp_name'] = emp['employee_name']
                ...
                affected_allocations.at[index, 'shortage_flag'] = f'Reallocated ({allocation_level})'
                affected_allocations.at[index, 'reallocation_level'] = allocation_level
                affected_allocations.at[index, 're_allocated_employee'] = emp['employee_name']
            ...
            if not employee_allocated:
                affected_allocations.at[index, 'shortage_flag'] = 'Reallocation Failed - No Capacity'
        else:
            affected_allocations.at[index, 'shortage_flag'] = 'Reallocation Failed - No Matching Employee'
...
def balance_by_operation_sequence(planned_manning_df, emp_fact_df, wip_df=None):
    ...
    return balance_by_operation_sequence_enhanced(planned_manning_df, emp_fact_df, wip_df)
...
def perform_dday_allocation_enhanced(allocation_date, attendance_df, emp_fact_df, wip_df=None,
    ...
    for idx in reallocated_manning.index:
        ...
        elif working_manning.loc[idx, 'shortage_flag'] in ["Partially Shortage", "Shortage Unresolved"]:
    ...
    shortage_rows = working_manning[working_manning['shortage_flag'].isin(["Partially Shortage", "Shortage Unresolved"])]
...
def generate_overtime_recommendations(manning_df, capacity_tracker, stats):
    ...
    shortages = manning_df[manning_df["shortage_flag"].str.contains("Shortage", na=False)]

    if shortages.empty:
        return pd.DataFrame()  # No shortages, no overtime needed

    # Calculate shortage statistics by code and line
    shortage_by_code_line = shortages.groupby(["line", "code", "section"])["planned_qty"].sum().reset_index()
    shortage_by_code_line = shortage_by_code_line.sort_values("planned_qty", ascending=False)
    ...
    for _, shortage in shortage_by_code_line.iterrows():
        ...
        matching_employees = manning_df[
            (manning_df["code"] == code) &
            (manning_df["allocated_emp_id"].notna())
            ...
            if not emp_details.empty:
                ...
                if utilization >= 75:
                    overtime_recommendations.append({
                        ...
                        "OVERTIME_HOURS_RECOMMENDED": min(4, math.ceil(qty / (emp_details.iloc[0]["PRIMARY CAPACITY"] / 8))),
    ...
    if overtime_recommendations:
        ...
        ot_df = ot_df.sort_values(["SHORTAGE_QTY", "CURRENT_UTILIZATION"], ascending=[False, False])
        return ot_df
...
def analyze_mass_absenteeism_new(attendance_df, allocation_date, emp_details):
    """
...
def analyze_and_save_mass_absenteeism(attendance_df, current_date, output_dir, emp_fact_df):
    """
    ...
    try:
        ...
        # Save results
        absence_path = os.path.join(output_dir, f"absenteeism_analysis_{current_date.strftime('%Y%m%d')}.csv")
        absence_rates_df.to_csv(absence_path, index=False)
        print(f"Saved absenteeism analysis to {absence_path}")
    except Exception as e:
        print(f"Warning: Could not save absenteeism analysis CSV: {e}")

    try:
        # Save summary to text file
        summary_path = os.path.join(output_dir, f"absenteeism_summary_{current_date.strftime('%Y%m%d')}.txt")
        ...
        print(f"Saved absenteeism summary to {summary_path}")
    except Exception as e:
        print(f"Warning: Could not save absenteeism summary text: {e}")
...
def run_intraday_allocation(allocation_date, attendance_df, emp_fact_df, run_times):
    ...
    """
    print("Warning: Using legacy intraday allocation function. Consider using run_intraday_allocation_enhanced instead.")
...
def get_prioritized_employees(emp_df, capacity_tracker, line=None, factory=None, floor=None, code=None, section=None, emp_type=None, designation=None):
    """
    ...
    for idx, row in emp_df_copy.iterrows():
        emp_id = row["employee_id"]
        code_value = row["code"]
        effective_remaining = capacity_tracker.get_effective_remaining_capacity(emp_id, code_value)
...
def find_preferred_employees(emp_fact_df, capacity_tracker, code=None, line=None, factory=None, floor=None, current_emp_id=None):
    """
    Find preferred employees using the same prioritization logic as get_prioritized_employees
    Returns a comma-separated string of employee names with their line
    Now also includes unassigned employees with matching skills
    ...
    # First try same line
    if line:
        same_line_employees = get_prioritized_employees(emp_fact_df, capacity_tracker, line=line, code=code)
        if not same_line_employees.empty:
            # Exclude current employee
            if current_emp_id is not None and not pd.isna(current_emp_id):
                same_line_employees = same_line_employees[same_line_employees["employee_id"] != current_emp_id]
            ...
            if not same_line_employees.empty:
                ...
                for i in range(len(same_line_employees)):
                    ...
                    preferred_same_line.append(f"{row['employee_name']} - {row['employee_id']} [Line: {row['line']}, Capacity: {capacity}]")

    # Try same factory
    if factory:
        same_factory_employees = get_prioritized_employees(emp_fact_df, capacity_tracker, factory=factory, code=code)
        if not same_factory_employees.empty:
            # Exclude current employee and those already in same line
            same_factory_employees = same_factory_employees[
                (same_factory_employees["employee_id"] != current_emp_id) &
            ...
            if not same_factory_employees.empty:
                for i in range(len(same_factory_employees)):
                    ...
                    preferred_same_factory.append(f"{row['employee_name']} - {row['employee_id']} [Line: {row['line']}, Capacity: {capacity}]")

    # Try same floor
    if floor:
        same_floor_employees = get_prioritized_employees(emp_fact_df, capacity_tracker, floor=floor, code=code)
        if not same_floor_employees.empty:
            # Exclude current employee and those from same line or factory
            if factory:
                same_floor_employees = same_floor_employees[
            ...
            if not same_floor_employees.empty:
                for i in range(len(same_floor_employees)):
                    ...
                    preferred_same_floor.append(f"{row['employee_name']} - {row['employee_id']} [Line: {row['line']}, Capacity: {capacity}]")

    # Try any location
    anywhere_employees = get_prioritized_employees(emp_fact_df, capacity_tracker, code=code)
    if not anywhere_employees.empty:
        # Exclude current employee and those already included in other categories
        if floor and factory:
            anywhere_employees = anywhere_employees[
        ...
        if not anywhere_employees.empty:
            for i in range(len(anywhere_employees)):
                ...
                preferred_anywhere.append(f"{row['employee_name']} - {row['employee_id']} [Line: {row['line']}, Capacity: {capacity}]")
...
def reallocate_work_enhanced(affected_allocations, emp_fact_df, allocation_date, wip_df=None, emp_type=None):
    """
    ...
    print(f"Capacity tracker initialization: {time.time() - start_time:.2f} seconds")
    ...
    for index, row in affected_allocations.iterrows():
        ...
        hierarchical_searches = [
            ...
            ({"line": line, "code": code, "emp_type": "Secondary" if emp_type == "Primary" else "Primary"}, "same_line_secondary"),
            ({"factory": factory, "code": code, "emp_type": emp_type}, "same_factory"),
            ({"factory": factory, "code": code, "emp_type": "Secondary" if emp_type == "Primary" else "Primary"}, "same_factory_secondary"),
            ({"floor": floor, "code": code, "emp_type": emp_type}, "same_floor"),
            ({"floor": floor, "code": code, "emp_type": "Secondary" if emp_type == "Primary" else "Primary"}, "same_floor_secondary"),
            ({"code": code, "emp_type": emp_type}, "other_location"),
            ({"code": code, "emp_type": "Secondary" if emp_type == "Primary" else "Primary"}, "other_location_secondary"),
        ]
        ...
        for search_args, level in hierarchical_searches:
            if available_employee.empty:
                ...
                if search_key in code_employee_cache:
                    ...
                    available_employee = get_prioritized_employees(emp_fact_df, capacity_tracker, **search_args)
        ...
        if not available_employee.empty:
            ...
            if not available_employee.empty:
                ...
                if 'effective_remaining_capacity' in available_employee.columns:
                    ...
                    effective_capacity = capacity_tracker.get_effective_remaining_capacity(emp_id, code)
                ...
                if allocation > 0:
                    ...
                    affected_allocations.loc[index, 'shortage_flag'] = 'Reallocation Failed - Insufficient Capacity'
            else:
                affected_allocations.loc[index, 'shortage_flag'] = 'Reallocation Failed - Already Allocated'
        else:
        ...
        if index % 10 == 0:  # Print every 10 allocations
            print(f"Processed allocation {index} in {time.time() - step_start:.2f} seconds")
    ...
    print(f"Total reallocation time: {time.time() - start_time:.2f} seconds")
    return affected_allocations, reallocation_tracking



def balance_by_operation_sequence_enhanced(planned_manning_df, emp_fact_df, wip_df=None):
    """
    Enhanced balancing function that considers WIP ratios, minimum staffing, and optimal buffers
...
def analyze_mass_absenteeism(attendance_df, allocation_date, emp_details):
    ...
    if 'last_updated' in daily_attendance.columns:
        ...
        latest_attendance = daily_attendance.groupby('employee_id', as_index=False).last()
    else:
        latest_attendance = daily_attendance.groupby('employee_id', as_index=False).first()
    ...
    date_range_mask = (attendance_df['attendance_date'] >= start_date) & (attendance_df['attendance_date'] <= end_date)
    ...
    if 'last_updated' in date_range_attendance.columns:
        date_range_attendance['last_updated'] = pd.to_datetime(
            date_range_attendance['last_updated'],
            errors='coerce'
        )
    ...
    try:
        ...
        by_line = merged_attendance.groupby('line')[status_col].apply(
            lambda x: (x == 'A').mean() * 100
        ).sort_values(ascending=False)

        by_section = merged_attendance.groupby('section')[status_col].apply(
            lambda x: (x == 'A').mean() * 100
        ).sort_values(ascending=False)

        by_floor = merged_attendance.groupby('floor')[status_col].apply(
            lambda x: (x == 'A').mean() * 100
        ).sort_values(ascending=False)
