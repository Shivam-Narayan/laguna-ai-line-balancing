...
def convert_to_excel_data(
    data, sheet_name, shortage_data=None, prediction=None, machinists_info=None
):
    """
    ...
    with pd.ExcelWriter(excel_data, engine="openpyxl") as writer:
        ...
        if prediction is not None:
            ...
            worksheet.merge_cells(
                start_row=1, start_column=1, end_row=1, end_column=heading_col_span
            )
        ...
        if shortage_data is not None:
            ...
            predicted_dict = {
                item["section"]: item["total_planned_qty"]
                for item in entry["predicted_production"]
            }
            target_dict = {
                item["section"]: item["total_planned_qty"]
                for item in entry["production_target"]
            }
            ...
            for i, section in enumerate(sections):
                ...
                worksheet.cell(
                    row=start_row + 3 + i,
                    column=2,
                    value=predicted_dict.get(section, 0),
                )
                worksheet.cell(
                    row=start_row + 3 + i, column=3, value=target_dict.get(section, 0)
                )
            ...
            for col_idx in range(1, heading_col_span + 1):
                ...
                for i in range(len(sections)):
                    values.append(
                        worksheet.cell(row=start_row + 3 + i, column=col_idx).value
                    )
        ...
        if machinists_info is not None:
            ...
            required_machinists_dict = {
                item["section"]: item["count"] for item in required_machinists_list
            }
            actual_machinists_dict = {
                item["section"]: item["count"] for item in actual_machinists_list
            }
            sections = sorted(
                set(required_machinists_dict.keys()).union(
                    actual_machinists_dict.keys()
                )
            )
            ...
            for i, section in enumerate(sections):
                ...
                worksheet.cell(
                    row=start_row + 3 + i,
                    column=2,
                    value=required_machinists_dict.get(section, 0),
                )
                worksheet.cell(
                    row=start_row + 3 + i,
                    column=3,
                    value=actual_machinists_dict.get(section, 0),
                )
...
def write_absenteeism_data_to_excel(data, filename):
    ...
    for line, line_data in data.items():
        ...
        total_predicted = sum(
            [item["count"] for item in line_data["predicted_absent_count"]]
        )
...
def export_absenteeism_predictions_excel():
    ...
    response["Content-Disposition"] = (
        'attachment; filename="Prediction_Data_2_Months.xlsx"'
    )
