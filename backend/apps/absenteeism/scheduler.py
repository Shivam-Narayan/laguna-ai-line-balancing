...
def start():
    ...
    if not scheduler_started:
        ...
        scheduler.add_job(
            run_absenteeism_prediction_task.delay,
            CronTrigger(
                hour=3, minute=30, day_of_week="mon-sat", timezone=ist
            ),  # 03:30 (03:30 AM)
        ...
        scheduler.add_job(
            scheduler_prediction_data_email_task.delay,
            CronTrigger(
                hour=9, minute=0, day_of_week="mon-sun", timezone=ist
            ),  # 9:00 (09:00 AM)
            ...
            args=[
                "all",
                1,
            ],  # first parameter is the Lines, second is the forecast period
        )
        scheduler.add_job(
            save_absenteeism_report_task.delay,
            CronTrigger(
                hour=19, minute=30, day_of_week="mon-sun", timezone=ist
            ),  # 19:30 (09:30 PM)
        ...
        scheduler.add_job(
            fetch_absenteeism_report_data_task.delay,
            CronTrigger(
                hour=13, minute=30, day_of_week="mon-sun", timezone=ist
            ),  # 13:30 (01:30 PM)
        ...
        scheduler.add_job(
            run_absenteeism_report_task.delay,
            CronTrigger(
                hour=18, minute=30, day_of_week="mon-sun", timezone=ist
            ),  # 18:30 (06:30 PM)
