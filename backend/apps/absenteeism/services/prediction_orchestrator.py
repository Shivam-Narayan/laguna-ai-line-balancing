...
from apps.manning_sheet.models import LoadingPlan, ManningSheetData
from apps.manning_sheet.utils import create_bulk_push_notifications
from apps.manning_sheet.views import NOTIFICATION_DISPLAY_TITLE
from config.utils import truncate_table

from ..models import AbsenteeismPrediction, PredictionData
from ..utils import (
    convert_number,
...
from .absenteeism_percentage_service import (
    calculate_line_percentages,
    get_working_days_around_date,
)
...
def run_absenteeism_prediction(viaAPI):
    try:
        ...
        if consolidated_df is None:
            return error_response(
                error="Prediction generation failed. Ensure there is enough historical attendance and weather data.",
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Convert 'datetime' column to date format
        consolidated_df["datetime"] = pd.to_datetime(
            consolidated_df["datetime"]
        ).dt.date
        ...
        consolidated_df["forecast_period"] = consolidated_df["forecast_period"].astype(
            int
        )

        # Map limit dates to a new column for vectorized comparison
        consolidated_df["limit_date"] = consolidated_df["forecast_period"].map(
            forecast_limits
        )
        ...
        for _, row in consolidated_df.iterrows():  # type: ignore
            ...
            if len(prediction_data_objects) >= batch_size:
                with transaction.atomic():  # type: ignore
                    AbsenteeismPrediction.objects.bulk_create(
        ...
        logger.error(
            f"Error in run_absenteeism_prediction: {str(e)} at {datetime.now()} hours!",
            exc_info=True,
        )
...
def prepare_prediction_data(line_no, forecast_period, summation=False):
    try:
        ...
        if line_no_lower not in valid_lines:
            return error_response(
                error="Invalid line number.", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if total_emp_count == 0:
            return error_response(
                error="No employees found.", status=status.HTTP_404_NOT_FOUND
            )
        ...
        if not isWorkingDay:
            return error_response(
                error=f"No data found for {filter_date} as it is {reason}",
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not prediction_qs.exists():
            return error_response(
                error="No predictions found.", status=status.HTTP_404_NOT_FOUND
            )

        manning_sheet_qs = ManningSheetData.objects.filter(**manning_sheet_filter)  # type: ignore
        manning_sheet_target = manning_sheet_qs.values(
            "section", "code", "style"
        ...
        result = [
            {"section": sec, "total_planned_qty": qty}
            for sec, qty in section_summary.items()
        ]
        ...
        section_data = (
            EmployeeMaster.objects.filter(employee_filter)
            .values("section")
        ...
        total_planned_qty = (
            LoadingPlan.objects.filter(**loading_plan_filter).aggregate(  # type: ignore
        ...
        total_gap_summary = (
            AbsenteeismPrediction.objects.filter(**absenteeism_filter)
            .exclude(section="nan")
            .values("section")
        ...
        section_emp_count = {item["section"]: item["count"] for item in section_data}
        ...
        for item in gap_summary_normalized:
            ...
            if sec_count > 0:
                absenteeism_percentage_by_section[item["section"]] = round(
                    (item["count"] / sec_count * 100), 1
                )
            else:
        ...
        absenteeism_percentage = (
            round((total_sum / total_emp_count) * 100, 1) if total_emp_count else 0
        )
        ...
        for op in total_operators:
            gap = next(
                (
                    g
                    for g in total_operators_gap
                    if g["section"].lower() == op["section"].lower()
                ),
                None,
            )
        ...
        if line_no_lower == "all":
            ...
            for line_index in range(1, 11):
                ...
                response_data = prepare_prediction_data(
                    individual_line, forecast_period, summation=True
                )
                ...
                if "data" in response_data and "Target data" in response_data["data"]:
                    prediction_data = response_data["data"]["Target data"][0][
                        "predicted_production"
                    ]
                    # Store by line number for aggregation
                    all_line_predictions[individual_line] = prediction_data
                    required_machinists_all.append(
                        response_data["data"]["required_machinists"]
                    )
                    actual_machinists_all.append(
                        response_data["data"]["actual_machinists"]
                    )
        ...
        total_operators = sorted(
            total_operators, key=lambda x: safe_index(x["section"])
        )
        total_operators_supply = sorted(
            total_operators_supply, key=lambda x: safe_index(x["section"])
        )
        total_operators_gap = sorted(
            total_operators_gap, key=lambda x: safe_index(x["section"])
        )
        ...
        for section, operations in grouped_result.items():
            ...
            required_machinists.append({"section": section, "count": machinist_count})
        ...
        required_machinists = sorted(
            required_machinists, key=lambda x: section_order.index(x["section"])
        )
        actual_machinists = sorted(
            actual_machinists, key=lambda x: section_order.index(x["section"])
        )
...
def run_get_absenteeism_forecast(forecast_period, line):
    try:
        ...
        for year in years_to_include:
            hist_target_date = todays_date.replace(year=year) + timedelta(
        ...
        attendance_query = PredictionData.objects.filter(
            date__in=list(all_needed_dates)
        )
        if line != "ALL":
            attendance_query = attendance_query.filter(department=line)
        if not attendance_query.exists():
            return Response(
                {"error": f"No attendance data found for {line}"}, status=404
            )
        ...
        for year, working_dates in year_to_dates.items():
            ...
            for work_date in working_dates:
                ...
                if data_for_date:
                    percentage = calculate_line_percentages(
                        data_for_date, emp_counts, line
                    )
                else:
                    ...
                    percentage = (
                        calculate_line_percentages(similar_data, emp_counts, line)
                        if similar_data
                        else 0.0
                    )

                year_results.append(
                    {"date": work_date.strftime("%Y-%m-%d"), "percentage": percentage}
                )
...
def run_absenteeism_prediction_data(
    line_no, forecast_period, get_all, is_export=False, export_type="", email=""
):
    try:
        ...
        try:
            ...
            return error_response(
                error='"forecast_period" must be an integer.',
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not is_export:
            ...
            if export_type == "excel":
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="Prediction_Data_{line_no}_{forecast_period}.xlsx"'
                )
                return response
                ...
                return success_response(
                    ...
                    data={"message": "File will be attached to the email."},
                )

            return error_response(
                error='Type should be either "excel" or "email"',
                status=status.HTTP_400_BAD_REQUEST,
            )
