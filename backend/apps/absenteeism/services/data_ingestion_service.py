...
from ..models import Absenteeism, AbsenteeismPrediction, PredictionData
...
def run_upload_absenteesim_data(file, month, year):
    ...
    if not month or not year:
        return error_response(
            error="No month or year provided.", status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if month not in month_list:
        return error_response(
            error=f"Month value required in this format {month_list}",
    ...
    try:
        ...
        if file.name.endswith(".csv"):
            ...
            return error_response(
                error="Unsupported file format.", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        columns_to_rename = {
            i: f"{i:02d}-{month}-{year}" for i in range(1, endDate + 1)
        }
        columns_to_rename.update(
            {str(i): f"{i:02d}-{month}-{year}" for i in range(1, endDate + 1)}
        )
        # Also handle potential float parsing like '1.0'
        columns_to_rename.update(
            {f"{i}.0": f"{i:02d}-{month}-{year}" for i in range(1, endDate + 1)}
        )
        ...
        melted_data["DOJ"] = pd.to_datetime(
            melted_data["DOJ"], format="%d-%b-%y", errors="coerce"
        )
        melted_data["DOJ"] = melted_data["DOJ"].where(
            melted_data["DOJ"].notnull(), None
        )

        # Convert 'Date' to YYYY-MM-DD format
        melted_data["Date"] = pd.to_datetime(
            melted_data["Date"], format="%d-%b-%Y", errors="coerce"
        )
        if melted_data["Date"].isnull().any():  # type: ignore
            return error_response(
                error="Invalid date values found in the data.",
                status=status.HTTP_400_BAD_REQUEST,
            )

        melted_data[["P", "WO", "H", "L", "Ab", "DP", "OT1"]] = melted_data[
            ["P", "WO", "H", "L", "Ab", "DP", "OT1"]
        ].fillna(0)
        ...
        for row in melted_data.itertuples(index=False):
            ...
            try:
                ...
                logger.info(
                    f"Error creating object for row: {row.Empcode}, Error: {str(e)}"
                )
        ...
        if absenteeism_objects:
            try:
                ...
                return success_response(
                    data=f"Data successfully uploaded {month} {year} and saved.",
                    status=status.HTTP_201_CREATED,
                )
            except Exception as e:
                logger.info(f"Error during bulk_create: {str(e)}")
                return error_response(
                    error=f"Error saving data to the database: {str(e)}",
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            if melted_data.empty:
                return error_response(
                    error="No valid data to save. The uploaded file contains no data rows.",
                    status=status.HTTP_400_BAD_REQUEST,
                )
            return error_response(
                error="No valid data to save. All rows had missing Empcode.",
                status=status.HTTP_400_BAD_REQUEST,
            )

    except Exception as e:
        return error_response(
            error=f"Error processing the data: {str(e)}",
            status=status.HTTP_400_BAD_REQUEST,
        )
