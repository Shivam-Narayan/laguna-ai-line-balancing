...
from apps.data_engine.models import AttendanceMaster, EmployeeMaster

from ..models import AbsenteeismPrediction
from ..utils import (
    is_allowed_working_day,
    write_absenteeism_data_to_csv,
)
...
def run_absenteeism_report(viaAPI):
    try:
        ...
        if not viaAPI:
            ...
            logger.info(
                f"Running Absenteeism Report generation at {str(datetime.now())} hours!"
            )
        ...
        if not os.path.exists(absenteeism_report):
            return error_response(
                error="Absenteeism report not found.", status=status.HTTP_404_NOT_FOUND
            )
        ...
        if not viaAPI:
            ...
            logger.info(
                f"Data sent via mail successfully at {str(datetime.now())} hours!"
            )
            logger.info("***************************************************\n\n")

            return success_response(
                message="Absenteeism Report sent via mail successfully.",
        ...
        logger.error(
            f"Error in run_absenteeism_report: {str(e)} at {datetime.now()} hours!",
            exc_info=True,
        )
    ...
    try:
        ...
        for line in range(1, 11):
            ...
            if "data" in response_data:
                absenteeism_percentage = response_data["data"].get(
                    "absenteeism_percentage", 0
                )
                total_operators = response_data["data"].get("total_operators", [])
                total_employee_count = {
                    item["section"]: item["count"] for item in total_operators
                }
                total_operators_gap = response_data["data"].get(
                    "total_operators_gap", []
                )
        ...
        return success_response(
            message="Absenteeism Report saved successfully.", status=status.HTTP_200_OK
        )
    except Exception as e:
        logger.info(f"Error in run_absenteeism_report: {str(e)}")
        logger.error(
...
def fetch_absenteeism_report_data():
    try:
        ...
        if not absenteeism_data:
            return error_response(
                error="No absenteeism data found.", status=status.HTTP_404_NOT_FOUND
            )
        ...
        for line in range(1, 11):
            ...
            attendance_summary = (
                ...
                .annotate(
                    ...
                    absent=Count(
                        Case(When(~Q(status="P"), then=1), output_field=IntegerField())
                    ),
                )
            )
            ...
            for entry in attendance_summary:
                actual_present_count.append(
                    {"section": entry["section"], "count": entry["present"]}
                )
                actual_absent_count.append(
                    {"section": entry["section"], "count": entry["absent"]}
                )
            ...
            if total_emp_count > 0:
                actual_absenteeism_percentage = round(
                    ((absent_emp_count / total_emp_count) * 100), 1
                )
            else:
            ...
            absenteeism_data[individual_line]["actual_absenteeism_percentage"] = (
        ...
        return success_response(
            message="Absenteeism report data fetched successfully.",
        ...
        logger.error(
            f"Error in fetch_absenteeism_report_data: {str(e)} at {datetime.now()} hours!",
            exc_info=True,
        )
        return error_response(error=str(e), status=status.HTTP_400_BAD_REQUEST)
...
def absenteeism_report(line_no, today):
    try:
        ...
        total_employee_count = {
            item["section"]: item["count"] for item in total_employee_list
        }
