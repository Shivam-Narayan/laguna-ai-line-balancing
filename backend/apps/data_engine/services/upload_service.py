...
def run_upload_attendance_file(file):
    if not file:
        return error_response(
            error="File is required", status=status.HTTP_404_NOT_FOUND
        )
        ...
        try:
            ...
            # Convert data types
            df["employee_id"] = (
                pd.to_numeric(df["employee_id"], errors="coerce").fillna(0).astype(int)
            )
            ...
            ).dt.time.replace({pd.NaT: None})
            df["early_departure"] = df["early_departure"].apply(
            ...
            return success_response(
                message="File uploaded and data saved successfully", status=201
            )
...
def run_add_local_holiday_calender(file):
    try:
        ...
        holiday_df["week"] = (
            holiday_df["date"].dt.isocalendar().week
        )  # ISO week number (1-53) # type: ignore
...
def run_add_payable_working_days():
    try:
        ...
        holiday_df = pd.DataFrame({"date": ["21-12-2024", "21-06-2025"]})
