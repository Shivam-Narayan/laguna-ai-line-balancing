...
from apps.accounts.models import User, PasswordResetToken, MultiSessionToken, EndpointLock


@admin.register(User)
class UserAdmin(BaseUserAdmin):
    list_display = ('email', 'username', 'user_type', 'location', 'department', 'status', 'created_at')
    ...
    fieldsets = (
        ...
        ('Personal Info', {'fields': ('username', 'phonenumber', 'location', 'department')}),
        ('Location', {'fields': ('latitude', 'longitude')}),
        ('Permissions', {'fields': ('user_type', 'status', 'send_mail', 'is_staff', 'is_superuser', 'groups', 'user_permissions')}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'username', 'password1', 'password2', 'user_type', 'location', 'department'),
        }),
    )
