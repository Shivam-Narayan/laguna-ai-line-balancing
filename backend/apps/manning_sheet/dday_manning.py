...
def find_preferred_employees(emp_fact_df, code=None, line=None, current_emp_id=None):
    ...
    preferred_list = [f"{row['employee_name']} ({row['remaining_capacity']})"
                      for _, row in preferred_emps.iterrows()]
    
    # Create a string with employee names, their line, and remaining capacity
    preferred_list = [f"{row['employee_name']} [line: {row['line']}]" #({row['REMAINING CAPACITY']})
                      for _, row in preferred_emps.iterrows()]
...
def reallocate_work(affected_allocations, emp_fact_df, allocation_date):
    ...
    for index, row in affected_allocations.iterrows():
        ...
        available_employee = get_prioritized_employees(emp_fact_df, line=line, code=code)
        allocation_level = 'same_line'

        if available_employee.empty:
            available_employee = get_prioritized_employees(emp_fact_df, factory=factory, code=code)
            allocation_level = 'same_factory'

        if available_employee.empty:
            available_employee = get_prioritized_employees(emp_fact_df, floor=floor, code=code)
        ...
        preferred_employees = find_preferred_employees(emp_fact_df, code=code, line=line, current_emp_id=current_emp_id)
        affected_allocations.at[index, 'PREFERRED_EMPLOYEES'] = preferred_employees
...
def perform_dday_allocation(allocation_date, attendance_df, emp_fact_df):
    ...
    stats = {
        ...
        'failed_reallocations': len(reallocated_manning[reallocated_manning['shortage_flag'] == 'Reallocation Failed']),
