...
from .models import DDayData, EmployeesOnHold, PushNotification

# Variables for OptaFloor APIS
SKILL_MATRIX_API_URL = (
    "https://optafloor-laguna.jaza-soft.com/v1/api/external/skill-matrix"
)
    ...
    with pd.ExcelWriter(excel_data, engine="openpyxl") as writer:
        for section, section_data in data.items():
            ...
            general_info_df.to_excel(
                writer, index=False, sheet_name=section, startrow=1
            )
            start_row = len(general_info_df) + 3  # Leave space after general info

            # Write table data below general info
            table_data_df.to_excel(
                writer, index=False, sheet_name=section, startrow=start_row
            )
            ...
            for col_idx, col_name in enumerate(general_info_df.columns):
                max_length = max(
                    len(str(col_name)),
                    *[len(str(x)) for x in general_info_df[col_name].astype(str)],
                )
            ...
            for col_idx, col_name in enumerate(table_data_df.columns):
                ...
                max_length = max(
                    len(str(col_name)),
                    *[len(str(x)) for x in table_data_df[col_name].astype(str)],
                )
                col_letter = get_column_letter(
                    col_idx + 1
                )  # Convert column index to Excel letter
...
def fetch_data(
    ...
    try:
        ...
        if is_array_of_objects(data):
            ...
            logger.info(
                f"Fetched page {page + 1} of {data.get('totalPages', 'Unknown')}"
            )

            if not data.get("last", True):
                return fetch_data(
                    all_data, page + 1, api_url, department, transform_df
    ...
    for item in raw_data:
        ...
        for product in products if products else [None]:
            for operation in item.get("operationList", []):
                ...
                dependencies = (
                    ", ".join(operation.get("dependencies", []))
...
def keep_first_valid_line(line_list):
    ...
    if isinstance(line_list, list):  # Ensure it's a list
        valid_only = [
            line for line in line_list if line in VALID_LINES
        ]  # Filter valid lines
        return (
            valid_only[0] if valid_only else None
        )  # Keep only the first valid element
    return None
...
def merge_dataframe(df_skill_matrix, df_operations):
    try:
        ...
        df_merged = df_skill_matrix.merge(
            df_operations, left_on="operationCode", right_on="code", how="left"
        )
        ...
        df_merged["AVERAGE CAPACITY"] = (
            df_merged["avgCapacity"].fillna(0) * 9
        )  # Multiply avgCapacity by 9
        df_merged["PEAK CAPACITY"] = (
            df_merged["peakCapacity"].fillna(0) * 9
        )  # Multiply peakCapacity by 9

        df_merged["AVERAGE PERFORMANCE"] = (
            df_merged["avgPerformance"].fillna(0) * 9
        )  # Multiply avgPerformance by 9
        df_merged["PEAK PERFORMANCE"] = (
            df_merged["peakPerformance"].fillna(0) * 9
        )  # Multiply peakPerformance by 9
        ...
        df_merged["FACTORY"] = df_merged["LINE"].apply(
            lambda x: (
                FACTORY_FLOOR_MAPPING.get(x, (None, None))[0]
        ...
        df_merged["FLOOR"] = df_merged["LINE"].apply(
            lambda x: (
                FACTORY_FLOOR_MAPPING.get(x, (None, None))[1]
                if pd.notna(x) and str(x).strip() != ""
                else None
            )
        )
...
def process_skill_matrix_data(df_skill_matrix):
    ...
    df_skill_matrix = df_skill_matrix[
        ~df_skill_matrix["type"].isin(["", " ", None]) & df_skill_matrix["type"].notna()
    ]
    ...
    df_skill_matrix["lines"] = df_skill_matrix.loc[:, "lines"] = df_skill_matrix[
        "lines"
    ].apply(
    ...
    # Remove rows where 'type' is blank (empty string or NaN)
    df_skill_matrix = df_skill_matrix[
        df_skill_matrix["type"].notna() & (df_skill_matrix["type"].str.strip() != "")
    ]

    # Replace '9A' with '9' and '9B' with '10'
    df_skill_matrix["lines"] = df_skill_matrix["lines"].replace(
        {"Line 9A": "Line 9", "Line 9B": "Line 10"}
    )
...
def export_json_to_excel(json_data):
    ...
    for section, table_data in json_data.get("table_data", {}).items():
        ...
        row += 1
        styles = (
            json_data.get("unique_styles", {}).get(section, {}).get("unique_styles", [])
        )
...
def process_styles(df_merged):
    ...
    for style_name, style_df in df_merged.groupby("Name"):
        ...
        style_df["Operation Sequence (Overall)"] = style_df["Operation Code"].map(
            overall_sequence_map
        )
        ...
        for section_name, group in style_df.groupby("section"):
            ...
            sorted_section_nodes, broken_edges_section = (
                topological_sort_with_mitigation(G_section)
            )
            section_seq_map = {
                code: seq + 1 for seq, code in enumerate(sorted_section_nodes)
            }

            temp_df = group.copy()
            temp_df["Operation Sequence (Section wise)"] = temp_df[
                "Operation Code"
            ].map(section_seq_map)
    ...
    def remove_employee(row):
        ...
        if pd.notna(final_allocation) and isinstance(preferred, str):
            ...
            if match:
                ...
                updated_preferred = re.sub(
                    r",\s*,", ",", updated_preferred
                )  # Double commas
                updated_preferred = re.sub(
                    r":\s*,", ":", updated_preferred
                )  # Comma right after colon
                updated_preferred = re.sub(
                    r",\s*\|", " |", updated_preferred
                )  # Comma before |
                updated_preferred = re.sub(
...
def fetch_attendance_data(line_no, today, yesterday):
    ...
    attendance_metrics = AttendanceMaster.objects.filter(filters).aggregate(
        ...
        present_today=Sum(
            Case(
                When(
                    attendance_date__gte=today_start,
                    attendance_date__lte=today_end,
                    status="P",
                    then=Value(1),
                ),
                default=Value(0),
        ...
        present_yesterday=Sum(
            Case(
                When(
                    attendance_date__gte=yesterday_start,
                    attendance_date__lte=yesterday_end,
                    status="P",
                    then=Value(1),
                ),
                default=Value(0),
    ...
    return {
        "message": "Success",
        "data": {"attendance_data": attendance, "attendance_stats": attendance_stats},
        "status": status.HTTP_200_OK,
    }
...
def transform_unallocated_to_on_hold_from_dict(unallocated_data_dicts):
    ...
    for ue in unallocated_data_dicts:
        ...
        employee_id = (
            str(ue.get("employee_id"))
            if ue.get("employee_id") is not None
            else "UnknownID"
        )
...
def update_sections(input_array, main_array):
    ...
    missing_sections = [
        section for section in main_array if section not in existing_sections
    ]

    input_array.extend(
        {"section": section, "total_planned_qty": 0} for section in missing_sections
    )
    return input_array
...
def remove_duplicate_employee_dicts(data):
    ...
    for entry in data:
        for emp_id, name in entry.items():
            seen[emp_id] = (
                name  # will overwrite duplicates, keeping the last occurrence
            )
...
def fetchMaxQtyDday(df):
    ...
    df_latest_ops = pd.merge(
        df, latest_ops, on=["style", "line", "section", "code"], how="inner"
    )
    ...
    grouped_qty_stats.columns = [
        ...
        "max_planned_qty",
        "sum_planned_qty",
        "min_allocated_capacity",
        "max_allocated_capacity",
        "sum_allocated_capacity",
        "op_seq",
    ]
