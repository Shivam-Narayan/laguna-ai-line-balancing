...
def process_df(df_load_plan, holiday_dates, last_date_of_year, payable_dates):
    ...
    df_load_plan = df_load_plan.drop(
        columns=[
            col for col in df_load_plan.columns if str(col).strip().lower() == "nan"
        ],
        errors="ignore",
    )
    df_load_plan = df_load_plan.dropna(axis=1, how="all")  # 44444444444444
    df_load_plan = df_load_plan[
        df_load_plan.drop(columns=["sheet_name"], errors="ignore").notna().any(axis=1)
    ]
    ...
    df_load_plan["Planned Qty"] = pd.to_numeric(
        df_load_plan["Planned Qty"], errors="coerce"
    )  # Convert to numeric manually
    ...
    df_load_plan["DEL DATE"] = df_load_plan["DEL DATE"].apply(
        lambda x: convert_del_date(x, last_date_of_year)
    )
    ...
    columns_to_check = [
        col
        for col in last_few_rows.columns
        if col
        not in [
            "DEL DATE",
            "MONTH CODE",
    ...
    columns_to_keep = [
        ...
        and any(isinstance(date, str) and date in col for date in valid_dates)
    ]
    ...
    capacity_per_day = last_few_rows_filtered[
        last_few_rows_filtered["QTY ORDER"] == "CAPACITY/DAY"
    ]
    ...
    # Split the Dates column by '-'
    final_df_new[["Date_Start", "Date_End"]] = final_df_new["Dates"].str.split(
        "-", expand=True
    )
    ...
    # Convert to datetime format and reformat as string dd/mm/yy
    final_df_new["Date_Start"] = pd.to_datetime(
        final_df_new["Date_Start"], format="%d/%m/%Y"
    ).dt.strftime("%d/%m/%y")
    final_df_new["Date_End"] = pd.to_datetime(
        final_df_new["Date_End"], format="%d/%m/%Y"
    ).dt.strftime("%d/%m/%y")

    # df_load_plan_14 = df_load_plan[df_load_plan['Week']==14]
    df_load_plan = divide_qts_per_day(
        df_load_plan, final_df_new, holiday_dates, payable_dates
    )
...
def divide_qts_per_day(df_load_plan, final_df_new, holiday_dates, payable_dates):
    ...
    for week, week_group in df_load_plan.groupby("Week"):
        ...
        per_day_qty = week_to_capacity.get(
            week, DEFAULT_CAPACITY
        )  # Default to DEFAULT_CAPACITY if week not found
        ...
        for _, row in week_group.iterrows():
            ...
            working_days = [
                ...
                if (
                    date.date()
                    in payable_dates  # Include if explicitly in PayableWorkingDays
                    or (
                        date.weekday() != 6
            ...
            if qty_processed < total_planned:  # New line
                logger.info(
                    f"Warning: Could not schedule all quantity for order {row['OC NO']}. Scheduled: {qty_processed}/{total_planned}"
                )
    ...
    if (
        abs(total_planned - total_scheduled) > 0.001
    ):  # Using small threshold to account for floating point errors
        logger.info(
            f"Warning: Not all quantities scheduled. Original: {total_planned}, Scheduled: {total_scheduled}"
        )
    ...
    if abs(total_qty_order_original - total_qty_order_scheduled) > 0.001:  # New line
        logger.info(
            f"Warning: QTY ORDER totals don't match. Original: {total_qty_order_original}, Scheduled: {total_qty_order_scheduled}"
        )
        logger.info(
            f"Difference: {total_qty_order_original - total_qty_order_scheduled}"
        )

    # Print daily capacity usage
    logger.info("\nDaily capacity usage:")
    daily_usage = (
        result_df.groupby(["Week", "Planned_Dates"])["Planned Qty"].sum().reset_index()
    )
