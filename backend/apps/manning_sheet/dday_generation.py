...
def check_floaters_for_allocation(
    ...
    if "FLOATER" in emp_fact_df.columns:
        ...
        query &= emp_fact_df["designation"].str.contains(
            "Floaters", case=False, na=False
        )
    else:
...
def find_preferred_employees(
    ...
    if line:
        same_line_employees = get_prioritized_employees(
            emp_fact_df, line=line, code=code, absent_employees=absent_set
        )
    ...
    if factory:
        same_factory_employees = get_prioritized_employees(
            emp_fact_df, factory=factory, code=code, absent_employees=absent_set
        )
    ...
    if floor:
        same_floor_employees = get_prioritized_employees(
            emp_fact_df, floor=floor, code=code, absent_employees=absent_set
        )
    ...
    anywhere_employees = get_prioritized_employees(
        emp_fact_df, code=code, absent_employees=absent_set
    )
    ...
    if handle_shortages:
        shortage_allocations = planned_manning_df[
            planned_manning_df["shortage_flag"].isin(
                ["Partial Shortage", "Shortage Unresolved"]
            )
        ].copy()
...
def balance_by_operation_sequence_enhanced(
    planned_manning_df, emp_fact_df, wip_df=None, absent_employees=None
):
    """
    ...
    for idx, row in balanced_manning.iterrows():
        ...
        if (
            pd.notna(emp_id)
            and emp_id not in absent_set
            and row.get("shortage_flag") != "Completed"
        ):
            ...
            if allocation_capacity > 0:
                if emp_id not in employee_allocation_tracker:
                    ...
                    employee_allocation_tracker[emp_id]["used_capacity"] += (
                        allocation_capacity
                    )
    ...
    logger.info(
        f"Balance operation: Initialized allocation tracker with {len(employee_allocation_tracker)} employees"
    )
    ...
    if wip_df is not None:
        ...
        if join_columns:
            ...
            if wip_qty_col:
                ...
                balanced_manning["wip_quantity"] = merged_df[wip_qty_col].astype(
                    float
                )  # Convert to float
    ...
    for idx, row in balanced_manning.iterrows():
        if pd.notna(row["allocated_capacity"]) and row["allocated_capacity"] > 0:
            ...
            balanced_manning.loc[idx, "estimated_completion_time"] = min(
                hours_needed, 9
            )  # Use .loc instead of .at
            ...
            balanced_manning.loc[idx, "optimal_wip_buffer"] = float(
                hourly_output * 2
            ...
            if hours_needed < 6 and row["wip_quantity"] > hourly_output * 1:
                ...
                balanced_manning.loc[idx, "excess_capacity"] = float(
                    excess_hours * (allocated_capacity / 8)
                )  # Explicitly cast to float
    ...
    for line, group in balanced_manning.groupby("line"):
        ...
        for i in range(len(line_ops) - 1):
            ...
            current_wip = balanced_manning.loc[
                current_op_idx, "wip_quantity"
            ]  # Use .loc instead of .at
            optimal_buffer = balanced_manning.loc[
                next_op_idx, "optimal_wip_buffer"
            ]  # Use .loc instead of .at

            if optimal_buffer > 0:
                # Calculate ratio of current WIP to optimal buffer
                wip_ratio = float(
                    current_wip / optimal_buffer
                )  # Explicitly cast to float
                balanced_manning.loc[next_op_idx, "wip_ratio"] = (
                    wip_ratio  # Use .loc instead of .at
                )

                # Categorize WIP status
                if wip_ratio > 1.5:
                    # More than enough WIP for next operation
                    balanced_manning.loc[current_op_idx, "wip_status"] = (
                        "EXCESS"  # Use .loc instead of .at
                    )
                elif wip_ratio < 0.5:
                    # Not enough WIP for next operation
                    balanced_manning.loc[current_op_idx, "wip_status"] = (
                        "SHORTAGE"  # Use .loc instead of .at
                    )
                else:
                    # Just right
                    balanced_manning.loc[current_op_idx, "wip_status"] = (
                        "OPTIMAL"  # Use .loc instead of .at
                    )
        ...
        if not same_line_donors.empty:
            ...
            for donor_idx, donor in same_line_donors.iterrows():
                ...
                if donor_emp_id in employee_allocation_tracker:
                    ...
                    if used_ratio > 0.8:
                        actual_remaining_capacity = actual_remaining_capacity * (
                            1.0 - used_ratio
                        )
                ...
                if emp_has_skill:
                    ...
                    transfer_capacity = min(
                        ...
                        recipient["planned_qty"]
                        / 4,  # At most enough for 2 hours of work
                    )
                    ...
                    if transfer_capacity > 0:
                        ...
                        balanced_manning.loc[donor_idx, "excess_capacity"] -= (
                            transfer_capacity  # Use .loc instead of .at
                        )

                        # If donor no longer has excess, remove from potential donors
                        if (
                            balanced_manning.loc[donor_idx, "excess_capacity"] <= 0
                        ):  # Use .loc instead of .at
                            balanced_manning.loc[donor_idx, "resource_status"] = (
                                "BALANCED"  # Use .loc instead of .at
                            )
                        ...
                        emp_data = emp_fact_df[
                            emp_fact_df["employee_id"] == donor_emp_id
                        ]
                        if (
                            emp_data.empty
                            or emp_data["remaining_capacity"].iloc[0] <= 0
                        ):
                        ...
                        transfer_capacity = min(
                            transfer_capacity, current_fact_capacity
                        )
                        ...
                        if pd.notna(prev_emp_id) and prev_emp_id != donor_emp_id:
                            ...
                            balanced_manning.loc[idx, "original_capacity"] = (
                                balanced_manning.loc[idx, "allocated_capacity"]
                            )
                        ...
                        balanced_manning.loc[idx, "resource_status"] = "BALANCED"
                        balanced_manning.loc[idx, "reallocation_reason"] = (
                            "WIP_optimization"
                        )
                        balanced_manning.loc[idx, "reallocation_level"] = (
                            "critical_shortage_prevention"
                        )
                        balanced_manning.loc[idx, "shortage_flag"] = (
                            "Reallocated via OP_SEQ"
                        )
                        ...
                        emp_fact_df.loc[
                            emp_fact_df["employee_id"] == donor_emp_id,
                            "remaining_capacity",
                        ] = max(0, current_fact_capacity - transfer_capacity)
    ...
    for line, group in balanced_manning.groupby("line"):
        ...
        for i in range(len(line_ops) - 1):
            ...
            if (
                ...
                and balanced_manning.loc[current_op_idx, "excess_capacity"] > 0
            ):
    ...
    for idx, row in balanced_manning.iterrows():
        ...
        if pd.notna(emp_id) and emp_id in absent_set:
            ...
            logger.info(
                f"WARNING: Row {idx} has absent employee {emp_id} assigned in balance operation"
            )
            ...
            balanced_manning.loc[idx, "shortage_flag"] = (
                "Reallocation Failed - Employee Absent"
            )

    if absent_allocations_count > 0:
        logger.info(
            f"Fixed {absent_allocations_count} absent employee allocations in balance operation"
        )
...
def reallocate_work_enhanced(
    ...
    if full_manning_df is not None:
        logger.info(
            f"Pre-populating capacity map with existing allocations from {len(full_manning_df)} rows"
        )
        ...
        for idx, row in full_manning_df.iterrows():
            ...
            if (
                ...
                if allocation_capacity > 0 and emp_id in employee_capacity_map:
                    # Allocate from consolidated capacity
                    employee_capacity_map[emp_id]["remaining_capacity"] -= (
                        allocation_capacity
                    )
                    employee_capacity_map[emp_id]["allocations"].append(
                        {
    ...
    # Update emp_fact_df with consolidated remaining capacities
    for emp_id, emp_data in employee_capacity_map.items():
        emp_fact_df.loc[emp_fact_df["employee_id"] == emp_id, "remaining_capacity"] = (
            emp_data["remaining_capacity"]
        )

    logger.info(
        f"Initialized consolidated capacity tracking for {len(employee_capacity_map)} employees"
    )
    ...
    for index, row in affected_allocations.iterrows():
        ...
        if pd.notna(current_emp_id) and current_emp_id in employee_capacity_map:
            ...
            if current_allocation > 0:
                # Free up capacity in consolidated map
                employee_capacity_map[current_emp_id]["remaining_capacity"] += (
                    current_allocation
                )
                ...
                employee_capacity_map[current_emp_id]["allocations"] = [
                    alloc
                    for alloc in employee_capacity_map[current_emp_id]["allocations"]
                    if alloc["row_idx"] != index
                ]

                # Update emp_fact_df
                emp_fact_df.loc[
                    emp_fact_df["employee_id"] == current_emp_id, "remaining_capacity"
                ] = employee_capacity_map[current_emp_id]["remaining_capacity"]

                logger.info(
                    f"Freed {current_allocation} capacity from employee {current_emp_id} for reallocation (row {index})"
                )

        # 1. Check floaters first
        available_employee = check_floaters_for_allocation(
            emp_fact_df, code=code, absent_employees=absent_set
        )
        allocation_level = "floater"
        ...
        if available_employee.empty:
            available_employee = get_prioritized_employees(
                emp_fact_df, code=code, emp_type=emp_type, absent_employees=absent_set
            )
            allocation_level = "other_location"
        ...
        if available_employee.empty and op_sequence and wip_df is not None:
            ...
            if not next_op_rows.empty:
                ...
                if pd.notna(next_op_emp_id) and next_op_emp_id not in absent_set:
                    ...
                    if next_op_emp_id in employee_capacity_map:
                        remaining_capacity = employee_capacity_map[next_op_emp_id][
                            "remaining_capacity"
                        ]
                    else:
                        # If not in capacity map, get from emp_fact_df
                    ...
                    if remaining_capacity > 0:
                        ...
                        if wip_df is not None:
                            ...
                            # Ensure boolean series
                            wip_filter = pd.Series(
                                [True] * len(wip_df), index=wip_df.index
                            )
                            for col in wip_key_cols:
                                wip_filter &= wip_df[col] == next_op_row[col]
                            ...
                            if not matching_wip.empty:
                                wip_qty_col = next(
                                    (
                                        col
                                        for col in matching_wip.columns
                                        if "WIP" in col.upper() and "QTY" in col.upper()
                                    ),
                                    None,
                                )
                                if wip_qty_col and matching_wip[wip_qty_col].sum() > 0:
        ...
        if not available_employee.empty:
            ...
            for i in range(len(available_employee)):
                ...
                if emp_id in absent_set:
                    logger.info(
                        f"WARNING: Found absent employee {emp_id} in available employees!"
                    )
                    continue
                ...
                if emp_id not in employee_capacity_map:
                    logger.info(
                        f"WARNING: Employee {emp_id} not found in capacity map!"
                    )
                    continue
                ...
                reallocation = {
                    ...
                    "planned_qty": planned_qty,
                    "allocated_qty": allocation,
                    "remaining_capacity": employee_capacity_map[emp_id][
                        "remaining_capacity"
                    ],
                    "operation_sequence": op_sequence,
                }
                ...
                # Update allocation details
                affected_allocations.loc[index, "allocated_emp_id"] = emp_id
                affected_allocations.loc[index, "allocated_emp_name"] = emp[
                    "employee_name"
                ]
                affected_allocations.loc[index, "allocated_capacity"] = allocation
                affected_allocations.loc[index, "allocated_frm_line"] = emp["line"]
                ...
                affected_allocations.loc[index, "machine"] = emp["machine"]
                affected_allocations.loc[index, "designation"] = emp["designation"]
                affected_allocations.loc[index, "target_100"] = (
                    planned_qty  # Set to planned quantity
                )
                affected_allocations.loc[index, "target_90"] = (
                    planned_qty * 0.9
                )  # Set to 90% of planned
    ...
    for emp_id, emp_data in employee_capacity_map.items():
        if emp_data["remaining_capacity"] < 0:
            over_allocated_count += 1
            logger.info(
                f"ERROR: Employee {emp_id} ({emp_data['name']}) is over-allocated! "
                f"Remaining capacity: {emp_data['remaining_capacity']}"
            )
            # Force fix
            emp_data["remaining_capacity"] = 0
    ...
    for idx, row in affected_allocations.iterrows():
        ...
        if pd.notna(emp_id) and emp_id in absent_set:
            absent_allocations_count += 1
            logger.info(
                f"WARNING: Row {idx} still has absent employee {emp_id} assigned"
            )
            planned_qty_for_target = row["planned_qty"]  # Get planned qty for this row
            affected_allocations.loc[idx, "allocated_emp_id"] = None
            affected_allocations.loc[idx, "allocated_emp_name"] = None
            affected_allocations.loc[idx, "allocated_capacity"] = 0
            affected_allocations.loc[idx, "shortage_flag"] = (
                "Reallocation Failed - Employee Absent"
            )
    ...
    if absent_allocations_count > 0:
        logger.info(
            f" {absent_allocations_count} allocations that still had absent employees assigned"
        )

    # Print allocation statistics
    logger.info(
        f"Allocation summary: {len(reallocation_tracking)} reallocations performed"
    )
    successful_count = len(
        affected_allocations[
            affected_allocations["shortage_flag"].str.contains("Reallocated", na=False)
        ]
    )
    failed_count = len(
        affected_allocations[
            affected_allocations["shortage_flag"].str.contains("Failed", na=False)
        ]
    )
...
def get_available_employees_with_skill(
    employee_capacity_map,
    code,
    emp_type=None,
    absent_employees=None,
...
def allocate_employee_capacity(
    employee_capacity_map, emp_id, allocation_amount, row_idx, line, section
):
    """
    ...
    if emp_data["remaining_capacity"] < allocation_amount:
        logger.info(
            f"WARNING: Insufficient capacity for employee {emp_id}. "
    ...
    logger.info(
        f"Allocated {allocation_amount} capacity to employee {emp_id}. "
...
def free_employee_capacity(employee_capacity_map, emp_id, allocation_amount, row_idx):
    ...
    for i, allocation in enumerate(emp_data["allocations"]):
        if (
            allocation["row_idx"] == row_idx
            and allocation["allocation"] == allocation_amount
        ):
            ...
            logger.info(
                f"Freed {allocation_amount} capacity from employee {emp_id}. "
                f"New remaining: {emp_data['remaining_capacity']}"
            )
            return True

    logger.info(
        f"WARNING: Could not find allocation to free for employee {emp_id}, "
...
def perform_dday_allocation_enhanced(
    ...
    for idx in balanced_manning.index:
        ...
        if balanced_manning.loc[idx, "shortage_flag"] == "Reallocated via OP_SEQ":
            ...
            if balanced_manning.loc[idx, "allocated_emp_id"] in absent_set:
                logger.info(
                    f"WARNING: Skipping OP_SEQ balancing for row {idx} - would allocate absent employee"
                )
                continue
            ...
            if pd.isna(final_manning.loc[idx, "original_emp_name"]):
                final_manning.loc[idx, "original_emp_name"] = final_manning.loc[
                    idx, "allocated_emp_name"
                ]
            ...
            final_manning.loc[idx, "allocated_emp_name"] = balanced_manning.loc[
                idx, "allocated_emp_name"
            ]
    ...
    for idx in final_manning.index:
        ...
        if pd.notna(emp_id) and emp_id in absent_set:
            ...
            final_manning.loc[idx, "shortage_flag"] = (
                "Reallocation Failed - Employee Absent"
            )

    if absent_allocations_count > 0:
        logger.info(
            f"Fixed {absent_allocations_count} allocations that still had absent employees assigned"
        )
...
def generate_unallocated_report(
    emp_fact_df, allocation_date, attendance_df=None, manning_df=None
):
    """
    ...
    if manning_df is None:
        logger.info(
            "Warning: No manning dataframe provided. Cannot determine unallocated employees."
        )
        return pd.DataFrame()
        ...
        for additional_emp_info in manning_df["additional_employees"].dropna():
            if pd.notna(additional_emp_info) and additional_emp_info != "":
                ...
                for emp_id in id_matches:
                    try:
                        allocated_additional.add(
                            float(emp_id)
                        )  # Convert to float to match emp_fact_df format
                    except ValueError:
                        allocated_additional.add(
                            emp_id
    ...
    for emp_id in unallocated_employee_ids:
        ...
        for idx, emp_record in emp_records.iterrows():
            ...
            unallocated_report = pd.concat(
                [unallocated_report, pd.DataFrame([report_record])], ignore_index=True
            )
...
def generate_unallocated_report_safe(
    emp_fact_df, allocation_date, attendance_df, manning_df
):
    """
    Safe wrapper that doesn't depend on remaining_capacity column
    """
    try:
        return generate_unallocated_report(
            emp_fact_df, allocation_date, attendance_df, manning_df
        )
    except Exception as e:
...
def determine_unallocation_reason(
    emp_id, emp_record, attendance_df=None, manning_df=None, allocation_date=None
):
    """
    ...
    if attendance_df is not None and allocation_date is not None:
        try:
            ...
            if not daily_attendance.empty:
                emp_attendance = daily_attendance[
                    daily_attendance["employee_id"] == emp_id
                ]
                if not emp_attendance.empty:
                    ...
                    if status == "A":
                        ...
                        "early_departure" in emp_attendance.columns
                        and emp_attendance["early_departure"].iloc[-1]
                    ):
...
def analyze_unallocated_patterns(unallocated_report):
    ...
    if unallocated_report.empty:
        return {"total_unallocated": 0, "message": "All employees have been allocated!"}
    ...
    # Analyze by reason
    reason_analysis = (
        unallocated_report.groupby("reason")
        .agg({"employee_id": "nunique"})
        .rename(columns={"employee_id": "Unique_Employees"})
        .sort_values("Unique_Employees", ascending=False)
    )

    # Analyze by line
    line_analysis = (
        unallocated_report.groupby("line")
        .agg({"employee_id": "nunique"})
        .rename(columns={"employee_id": "Unique_Employees"})
        .sort_values("Unique_Employees", ascending=False)
    )
    ...
    skill_analysis = (
        unallocated_report.groupby("code")
        .agg({"employee_id": "nunique"})
        .rename(columns={"employee_id": "Unique_Employees"})
        .sort_values("Unique_Employees", ascending=False)
    )
    ...
    type_analysis = (
        unallocated_report.groupby("type")
        .agg({"employee_id": "nunique"})
        .rename(columns={"employee_id": "Unique_Employees"})
        .sort_values("Unique_Employees", ascending=False)
    )
    ...
    multi_skill_employees = multi_skill_unallocated[
        multi_skill_unallocated > 1
    ].sort_values(ascending=False)
    ...
    analysis_summary = {
        ...
        "multi_skill_employees": multi_skill_employees.head(10).to_dict()
        if not multi_skill_employees.empty
        else {},
    }
...
def print_unallocated_summary(analysis_summary):
    ...
    logger.info(
        f"Total Unallocated Records: {analysis_summary['total_unallocated_records']}"
    )
    logger.info(
        f"Unique Unallocated Employees: {analysis_summary['unique_unallocated_employees']}"
    )
    ...
    if analysis_summary["multi_skill_employees"]:
        ...
        logger.info("-" * 50)
        for emp_id, skill_count in list(
            analysis_summary["multi_skill_employees"].items()
        )[:5]:
...
def analyze_allocation_gaps(manning_df, unallocated_df):
    ...
    unmet_operations["unmet_qty"] = unmet_operations["planned_qty"] - unmet_operations[
        "allocated_capacity"
    ].fillna(0)
    unmet_by_skill = (
        unmet_operations.groupby("code")["unmet_qty"].sum().sort_values(ascending=False)
    )

    # Find available capacity by skill code
    available_by_skill = (
        unallocated_df[unallocated_df["remaining_capacity"] > 0]
        .groupby("code")["remaining_capacity"]
        .sum()
        .sort_values(ascending=False)
    )
    ...
    return {
        ...
        "total_potential_additional_allocation": sum(
            [match["potential_fulfillment"] for match in potential_matches.values()]
        ),
    }
...
def perform_final_allocation_pass(manning_df, emp_fact_df, absent_employees=None):
    ...
    """
    logger.info(
        "Starting final allocation pass to maximize planned quantity fulfillment..."
    )
    ...
    for work_item in unmet_work:
        ...
        if available_employee.empty:
            available_employee = get_prioritized_employees(
                emp_fact_df, code=required_code, absent_employees=absent_employees
            )
        ...
        if not available_employee.empty:
            ...
            if allocation_amount > 0:
                ...
                if pd.isna(current_allocation) or current_allocation == 0:
                    ...
                    enhanced_manning.loc[row_idx, "allocated_emp_name"] = best_emp[
                        "employee_name"
                    ]
                    enhanced_manning.loc[row_idx, "allocated_capacity"] = (
                        allocation_amount
                    )
                    enhanced_manning.loc[row_idx, "allocated_frm_line"] = best_emp[
                        "line"
                    ]
                    enhanced_manning.loc[row_idx, "allocated_frm_factory"] = best_emp[
                        "factory"
                    ]
                    enhanced_manning.loc[row_idx, "allocated_frm_floor"] = best_emp[
                        "floor"
                    ]
                    enhanced_manning.loc[row_idx, "shortage_flag"] = (
                        "Final Pass Allocation"
                    )
                    enhanced_manning.loc[row_idx, "reallocation_level"] = (
                        "final_optimization"
                    )
                    enhanced_manning.loc[row_idx, "reallocation_reason"] = (
                        "maximize_planned_quantity"
                    )

                else:
                    # Existing allocation - increase it (multi-employee allocation)
                    enhanced_manning.loc[row_idx, "allocated_capacity"] = (
                        current_allocation + allocation_amount
                    )
                    enhanced_manning.loc[row_idx, "shortage_flag"] = (
                        "Multi-Employee Allocation"
                    )
                    enhanced_manning.loc[row_idx, "reallocation_level"] = (
                    ...
                    enhanced_manning.loc[row_idx, "additional_employees"] = (
                        additional_emp_info
                    )
                ...
                logger.info(
                    f"Additional allocation: {best_emp['employee_name']} -> Row {row_idx}, Capacity: {allocation_amount}"
                )

    logger.info("Final allocation pass completed:")
