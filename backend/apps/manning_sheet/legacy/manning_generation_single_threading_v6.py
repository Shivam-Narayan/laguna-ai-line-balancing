...
def filter_by_date_ranges_v6(df, today, date_thresholds, period):
    ...
    mapped_values = pd.DataFrame(
        df["line"].apply(map_factory_floor).tolist(),
        index=df.index,
        columns=["FACTORY", "FLOOR"],
    )
...
def process_single_manning_df_v6(df, df_Style_OB):
    ...
    first_appearance = first_appearance.rename(
        columns={"_original_order": "_group_order"}
    )
    ...
    manning = (
        ...
        .apply(lambda x: x.sort_values(by=["_original_order"]))
    ...
    manning = manning.merge(
        first_appearance,
        on=["oc_no", "order_no", "buyer", "style", "color", "line", "section"],
    )
    manning = (
        manning.sort_values("_group_order")
        .drop(columns=["_original_order", "_group_order"])
        .reset_index(drop=True)
    )
...
def run_manning_allocation_v6(
    PERIOD, manning_df, emp_fact_df, df_load_plan_transformed
):
    """
        ...
        if unallocated_all_periods:
            ...
            employee_cross_training = (
                skillset_not_required.groupby(
                    ["employee_id", "EMPLOYEE NAME", "LINE", "CODE"]
                )
                .size()
...
def process_manning_dataframe(manning_df, emp_fact_df, period):  # df_unique_smv=None
    ...
    for date in sorted(unique_dates):
        ...
        logger.info(
            f"Processing groups in original order: {[f'{line}/{section}/{code}' for line, section, code in ordered_groups]}"
        )
    ...
    if all_processed_rows:
        ...
        if "_source_original_index" in updated_manning_df.columns:
            updated_manning_df = updated_manning_df.sort_values(
                "_source_original_index"
            )
            # Clean up the temporary column
            updated_manning_df = updated_manning_df.drop(
                "_source_original_index", axis=1
            )
        ...
        for _, row in sorted_orders.iterrows():
            ...
            while remaining_qty > 0 and current_emp_index < len(employee_allocations):
                ...
                if order_allocation > 0:
                    ...
                    ##########################added 17-06-2025############################
                    current_row["_source_original_index"] = row[
                        "_global_original_index"
                    ]
