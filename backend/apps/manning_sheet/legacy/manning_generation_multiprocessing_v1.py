...
def update_employee_capacity_tracking(
    ...
    total_used = sum(employee_used_capacity[emp_id].values())
    ...
    for idx, row in emp_fact_df[emp_fact_df["employee_id"] == emp_id].iterrows():
        ...
        skill_key_current = f"{current_skill_type}_{current_code}"

        # Calculate effective capacity (limited by Primary capacity)
        effective_capacity = min(skill_capacity, primary_capacity)

        # If employee is working on this specific skill
        if skill_key_current in employee_used_capacity[emp_id]:
            used_for_this_skill = employee_used_capacity[emp_id][skill_key_current]
            remaining = max(0, effective_capacity - used_for_this_skill)
        else:
            # Employee is working on different skill, so this skill is unavailable
            if total_used > 0:
                remaining = 0
            else:
                remaining = effective_capacity

        emp_fact_df.at[idx, "remaining_capacity"] = remaining
...
def get_prioritized_employees(
    ...
    for idx, row in emp_df_copy.iterrows():
        ...
        # Check current usage for this employee
        emp_usage = employee_used_capacity.get(emp_id, {})

        # Calculate remaining capacity based on new logic
        if emp_usage:  # Employee has some allocation
            skill_key = f"{skill_type}_{skill_code}"

            # Check if employee is working on this specific skill
            if skill_key in emp_usage:
                # Employee is working on this skill, calculate remaining for this skill
                used_for_this_skill = emp_usage[skill_key]
                remaining = max(0, effective_capacity - used_for_this_skill)
            else:
                # Employee is working on different skill, this skill is unavailable
                total_used = sum(emp_usage.values())
                if total_used > 0:
                    remaining = 0  # Skills are mutually exclusive
                else:
                    remaining = effective_capacity  # Shouldn't happen, but safety check
        else:
            # Employee has no allocation yet, full effective capacity available
            remaining = effective_capacity
    ...
    filtered_df = filtered_df.sort_values(
        ...
        ascending=[
            True,
            False,
        ],  # Primary first (True because "Primary" < "Secondary"), then highest capacity
    )
...
def process_order_group(
    ...
    for _, emp in available_employees.iterrows():
        ...
        available_capacity = emp["effective_remaining_capacity"]
        ...
        if available_capacity > 0 and remaining_total_qty > 0:
            ...
            if allocation > 0:
                ...
                # NEW LOGIC: Use the new capacity tracking function
