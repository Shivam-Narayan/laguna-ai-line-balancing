        ...
        try:
            ...
            if manning_df is not None:
                ...
                updated_manning_df, unallocated_employees = process_manning_dataframe(
                    manning_df, emp_fact_df_original.copy(), period, df_unique_smv
                )
    ...
    try:
        if all_processed_dfs:
            ...
            for i in range(0, len(data_dicts), CHUNK_SIZE):
                ...
                logger.info(
                    f"Inserted chunk {i // CHUNK_SIZE + 1}/{(len(data_dicts) - 1) // CHUNK_SIZE + 1} with {len(chunk_dicts)} records"
                )
...
def process_manning_dataframe(manning_df, emp_fact_df, period, df_unique_smv=None):
    ...
    for date in sorted(unique_dates):
        ...
        emp_fact_df["remaining_capacity"] = emp_fact_df["remaining_capacity"].round(
            2
        )  # ----ROUND TO 1 PREFERRED
