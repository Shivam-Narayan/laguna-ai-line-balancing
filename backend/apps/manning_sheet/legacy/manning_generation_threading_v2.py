...
def process_single_manning_df(key, df, df_Style_OB):
    ...
    manning = manning.drop_duplicates()
...
def process_date_for_pool(args):
    ...
    date_df, unallocated = process_single_date_standalone(
        filtered_df, emp_fact_df.copy(), period
    )

    return date_df, unallocated


def process_manning_dataframe(
    manning_df, emp_fact_df, period, df_unique_smv=None, max_workers=None
):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for date_idx, (date_df, unallocated) in enumerate(
            executor.map(process_date_for_pool, data_by_date)
        ):
...
def _group_and_aggregate(shortages):
    ...
    grouped = grouped.sort_values(by="PLANNED_QTY", ascending=False)
...
def process_period(period_data_list, reference_data):
    period, df_manning = period_data_list
    ...
    df_merged["Median_Average_Capacity"] = df_merged["Median_Average_Capacity"].fillna(
        df_merged["Section_Average_Capacity"]
    )
    df_merged["Median_Average_Capacity"] = df_merged["Median_Average_Capacity"].replace(
        0, 1
    )
...
def process_general_info(manning_sheets, df_emp_fact):
    ...
    df_section_avg_capacity = df_emp_fact.groupby(["section", "line"], as_index=False)[
        "average_capacity"
    ].median()
    ...
    period_data_list = list(manning_sheets.items())
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Create a partial function with fixed reference_data
        process_func = partial(process_period, reference_data=reference_data)
        # Map the function to all periods
        for result in executor.map(process_func, period_data_list):
