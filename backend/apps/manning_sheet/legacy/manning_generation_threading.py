...
def process_grouped_results(filtered_dfs):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(process_single_df, key, df)
...
def create_manning_dataframes(result_dfs, df_Style_OB):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(process_single_manning_df, key, df, df_Style_OB)
            for key, df in result_dfs.items()
        ]
...
def get_prioritized_employees(
    emp_df, line=None, code=None, section=None, emp_type=None
):
    """
...
def run_manning_allocation(
    manning_dataframes_dict, emp_fact_df_original, df_unique_smv=None
):
    """
    ...
    manning_dataframes = [
        {
            "suffix": str(period),
            "period": period,
            "df": manning_dataframes_dict.get(str(period)),
        }
        ...
        try:
            ...
            if manning_df is not None:
                ...
                updated_df, unallocated = process_manning_dataframe(
                    manning_df, emp_fact_df_original, period, df_unique_smv
                )
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(process_wrapper, info) for info in manning_dataframes
        ]
        for future in as_completed(futures):
    ...
    unallocated_results = process_unallocated_data(
        unallocated_collection, manning_dataframes
    )
    ...
    try:
        if all_processed_dfs:
            ...
            consolidated_df["MACHINE_TYPE"] = consolidated_df["MACHINE_TYPE"].replace(
                "nan", "Not Applicable"
            )
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                data_dicts = list(
                    executor.map(
                        row_to_dict, [row for _, row in consolidated_df.iterrows()]
                    )
                )
...
def process_date_allocation(date, manning_df, emp_fact_df_base, period):
    ...
    """
    emp_fact_df = emp_fact_df_base.copy(
        deep=True
    )  # Deep copy to isolate capacity for each date

    # Reset employee capacity
    emp_fact_df["remaining_capacity"] = (
        emp_fact_df["average_capacity"].clip(lower=0).round(2)
    )
...
def process_manning_dataframe(manning_df, emp_fact_df, period, df_unique_smv=None):
    ...
    emp_fact_df_base = emp_fact_df.copy(
        deep=True
    )  # Base copy to isolate per-thread edits

    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(
                process_date_allocation, date, manning_df, emp_fact_df_base, period
            )
...
def process_order_group(line, section, code, group_orders, emp_fact_df, date, period):
    ...
    def build_shortage_reason():
        ...
        if len(other_lines) > 0:
            return (
                f"CODE={code} found only in lines: {', '.join(map(str, other_lines))}"
            )
    ...
    if available_employees.empty:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            new_rows = list(
                executor.map(
                    process_shortage_row, [r for _, r in group_orders.iterrows()]
                )
            )
        return new_rows
    ...
    for _, row in sorted_orders.iterrows():
        ...
        while remaining_qty > 0 and current_emp_index < len(employee_allocations):
            ...
            if order_allocation > 0:
                ...
                current_emp["ALLOCATION"] = round(
                    current_emp["ALLOCATION"] - order_allocation, 2
                )
        ...
        if remaining_qty > 0:
            ...
            shortage_row["SHORTAGE_REASON"] = (
                f"Insufficient capacity: Needed {round(remaining_qty, 1)} more units"
            )
            new_rows.append(shortage_row)
...
def get_unallocated_employees(emp_fact_df, daily_orders, date, period):
    ...
    def process_employee(emp):
        ...
        utilization_pct = round(
            (initial_capacity - remaining_capacity) / initial_capacity * 100, 2
        )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        results = list(
            executor.map(process_employee, [row for _, row in emp_fact_df.iterrows()])
        )
...
def process_unallocated_data(unallocated_collection, manning_dataframes):
    ...
    if unallocated_all_periods:
        ...
        all_unallocated["DATE"] = pd.to_datetime(
            all_unallocated["DATE"], errors="coerce"
        ).dt.strftime("%Y-%m-%d")
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            data_dicts = list(
                executor.map(
                    row_to_dict, [row for _, row in all_unallocated.iterrows()]
                )
            )
...
def process_general_info(manning_sheets, df_emp_fact):
    ...
    df_emp_fact_filtered = df_emp_fact[
        df_emp_fact["type"].isin(["Primary", "Secondary"])
    ]
    ...
    def process_period(period, df_manning):
        ...
        df_merged["Median_Average_Capacity"].fillna(
            df_merged["Section_Average_Capacity"], inplace=True
        )
        ...
        df_merged["Machinist_Required"] = df_merged.apply(
            lambda row: (
                row["Total_Operators_Required"] if row["code"] in machinist_codes else 0
            ),
            axis=1,
        )
        df_merged["Non_Machinist_Required"] = df_merged.apply(
            lambda row: (
                ...
                if row["code"] not in machinist_codes
                else 0
            ),
            axis=1,
        )
    ...
    data_dicts = df_final_Information.apply(
        lambda row: {
            ...
            "median_average_capacity": safe_get(
                row["Median_Average_Capacity"] if row["Median_Average_Capacity"] else 0
            ),
            ...
            "machinist_required": safe_get(
                float(row["Machinist_Required"]) if row["Machinist_Required"] else 0
            ),
