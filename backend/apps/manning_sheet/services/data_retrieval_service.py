...
def run_get_manning_data(
    line_no, section_value, period, style, planned_date, is_export=False
):
    """
    ...
    try:
        ...
        if not line_no or not section_value or not period:
            return error_response(
                error='"line", "section" and "forecast_period" are required.',
        ...
        if section_value not in valid_sections:
            return error_response(
                error="Invalid section. Choose from valid options.",
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        manning_sheet_filters = {
            "section": section_value,
            "planned_dates__in": date_range,
        }
        manning_general_filters = {
            "section": section_value,
            "planned_dates__in": date_range,
        }
        ...
        employees_on_hold_queryset = EmployeesOnHold.objects.filter(
            **employees_on_hold_filter
        )

        # Query filtered data
        filtered_data_table = ManningSheetData.objects.filter(
            **manning_sheet_filters
        ).distinct()
        filtered_data_info = ManningGeneralInfo.objects.filter(
            **manning_general_filters
        ).distinct()
        ...
        filtered_data_table = filtered_data_table.order_by(
        ...
        formatted_data = [
            {
                ...
                "Style": row["raw_style"],
                "Buyer": row["buyer"],
                "Color": row["raw_color"].upper()
                if row["raw_color"]
                else row["raw_color"],
                "OC Number": row["raw_oc_no"].upper()
                if row["raw_oc_no"]
                else row["raw_oc_no"],
        ...
        for row in formatted_data:
            ...
            if row.get("Operator ID") == 0:
                ...
                preferred_emps = [
                    emp
                    for emp in unique_preferred_employees
        ...
        actual_machinists = EmployeeMaster.objects.filter(
            **employee_master_filters
        ).count()
        machine_dict = {
            entry["machine_type"]: entry["count"] for entry in machine_nonMachine_info
        }

        unique_buyers = ", ".join(
            set(
                buyer.upper()
                for buyer in filtered_data_table.values_list("buyer", flat=True)
                if buyer
            )
        )
        info = {"buyers": unique_buyers}
        general_info = {
            "total_required": required_machinists,
            "total_available": actual_machinists,
        }
        ...
        if style.lower() == "all":
            unique_styles = filtered_data_table.values_list(
                "style", flat=True
            ).distinct()
            response_data["unique_styles"] = list(
                {s.upper() for s in unique_styles if s}
            )
...
def get_dday_data():
    try:
        ...
        planned_attendance = attendance_data["data"]["attendance_data"][
            "Planned Attendance"
        ]
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            for row_num in range(6, 6 + len(df)):  # Data starts from row 6
                worksheet.write(
                    row_num,
                    preferred_col_index,
                    df["Preferred Employees"].iloc[row_num - 6],
                    truncate_format,
                )
        ...
        notification_title = NOTIFICATION_DISPLAY_TITLE.get(
            notification_type, "Unknown"
        )

        with open(
            f"exports/Dday_Manning_data_ALL_{today}_{time_display.replace(' ', '_').replace(':', '_')}.xlsx",
            "wb",
        ) as f:
        ...
        create_bulk_push_notifications(
            ...
            data={
                "fileName": f"Dday_Manning_data_ALL_{today}_{time_display.replace(' ', '_').replace(':', '_')}.xlsx"
            },
        )
...
def get_dday_actual_vs_planned_data(
    line_no, today, section=None, operation=None, operation_code=None
):
    """
    ...
    try:
        ...
        def get_data_for_line(line: str) -> dict:
            ...
            total_planned_qty = (
                LoadingPlan.objects.filter(**lp_filter).aggregate(
                    total_planned_qty=Sum("planned_qty")
                )
            )["total_planned_qty"] or 0
            ...
            predicted_production = sum(
                item["min_allocated"] for item in style_min_dict.values()
            )
        ...
        if is_all_lines:
            ...
            response_data = {
                "Target data": {
                    ...
                    "style_breakdown": line_data[
                        "style_breakdown"
                    ],  # Include style breakdown
                }
            }
...
def run_get_unallocated_employees(line_no, forecast_period, is_export=False):
    try:
        ...
        today = datetime.today().date()
        date_range = [
            (today + timedelta(days=i)) for i in range(1, forecast_period + 1)
        ]  # This list won't include today's date
        ...
        if is_export:
            ...
            for col in df_unallocated_employees.select_dtypes(
                include=["datetimetz"]
            ).columns:
                df_unallocated_employees[col] = df_unallocated_employees[
                    col
                ].dt.tz_localize(None)
            # Generate Excel file in memory
            output = BytesIO()
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                df_unallocated_employees.to_excel(
                    writer, index=False, sheet_name="Unallocated Employees"
                )
            ...
            response["Content-Disposition"] = (
                "attachment; filename=unallocated_employees.xlsx"
            )
            return response
    except Exception as e:
...
def run_get_unallocated_employees_dday(line_no, is_export=False):
    try:
        ...
        if not os.path.exists(file_path):
            return error_response(
                error="D-Day unallocated report has not been generated yet. Please run D-Day generation first.",
        ...
        if line_no.lower() != "all":
            ...
            file_name = (
                f"Unallocated_Employees_DDay_{line_no.replace(' ', '_').title()}"
            )
