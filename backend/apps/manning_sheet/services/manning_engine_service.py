...
from ..dday_generation import (
    analyze_allocation_gaps,
    analyze_unallocated_patterns,
    generate_reallocation_report,
    generate_unallocated_report_safe,
    get_ist_time,
    perform_final_allocation_pass,
    print_unallocated_summary,
    run_intraday_allocation_enhanced,
)
from ..manning_generation_multiprocessing import (
    create_manning_dataframes,
    filter_by_date_ranges,
    map_factory_floor_for_results,
    process_general_info,
    process_grouped_results,
    run_manning_allocation,
)
from ..models import (
    ActiveEmployees,
    DDayData,
...
def manning_sheet_generation(request):
    try:
        ...
        df_unique_smv = df_load_plan_transformed.drop_duplicates(
            subset=["style", "buyer"]
        )
        ...
        df_load_plan_transformed["planned_dates"] = pd.to_datetime(
            df_load_plan_transformed["planned_dates"]
        ).dt.normalize()
        ...
        df_load_plan_transformed["planned_dates"] = pd.to_datetime(
            df_load_plan_transformed["planned_dates"], errors="coerce"
        )
        df_load_plan_transformed["planned_dates"] = df_load_plan_transformed[
            "planned_dates"
        ].dt.date  # Create four separate filtered DataFrames
        ...
        df_0_day = df_load_plan_transformed[
            df_load_plan_transformed["planned_dates"] == today
        ]

        # Group by ORDER NO, MERCHANT, STYLE and sum Planned Qty for each period
        result_60_days = df_60_days.groupby(
            [
                "oc_no",
                "order_no",
                "buyer",
        ...
        result_30_days = df_30_days.groupby(
            [
                "oc_no",
                "order_no",
                "buyer",
        ...
        result_7_days = df_7_days.groupby(
            [
                "oc_no",
                "order_no",
                "buyer",
        ...
        for df in [
            ...
            if not df.empty:
                df[["FACTORY", "FLOOR"]] = pd.DataFrame(
                    df["line"].apply(map_factory_floor).tolist(), index=df.index
                )
            else:
                df[["FACTORY", "FLOOR"]] = [
                    ["Unknown", "Unknown"]
                ]  # ✅ Assign default values to empty DataFrames
        ...
        manning_7 = (
            ...
            .apply(lambda x: x.sort_values(by=["planned_dates", "op_seq"]))
        ...
        # Ensure sorting by STYLE, section, and op_seq first, then by OC NO, ORDER NO, Line
        manning_30 = manning_30.sort_values(
            by=["style", "color", "section", "op_seq", "oc_no", "order_no", "line"]
        )

        # Drop Matched Style and style columns
        manning_30 = manning_30.drop(
            columns=["Matched Style", "UNNAMED: 0"], errors="ignore"
        )
        ...
        # Ensure sorting by STYLE, section, and op_seq first, then by OC NO, ORDER NO, Line
        manning_60 = manning_60.sort_values(
            by=["style", "color", "section", "op_seq", "oc_no", "order_no", "line"]
        )

        # Drop Matched Style and style columns
        manning_60 = manning_60.drop(
            columns=["Matched Style", "UNNAMED: 0"], errors="ignore"
        )
        ...
        # Ensure sorting by STYLE, section, and op_seq first, then by OC NO, ORDER NO, Line
        manning_1 = manning_1.sort_values(
            by=["style", "color", "section", "op_seq", "oc_no", "order_no", "line"]
        )

        # Drop Matched Style and style columns
        manning_1 = manning_1.drop(
            columns=["Matched Style", "UNNAMED: 0"], errors="ignore"
        )
        ...
        # Ensure sorting by STYLE, section, and op_seq first, then by OC NO, ORDER NO, Line
        manning_0 = manning_0.sort_values(
            by=["style", "color", "section", "op_seq", "oc_no", "order_no", "line"]
        )

        # Drop Matched Style and style columns
        manning_0 = manning_0.drop(
            columns=["Matched Style", "UNNAMED: 0"], errors="ignore"
        )
        ...
        for df_info in manning_dataframes:
            ...
            try:
                ...
                manning_df["PLANNED_DATES"] = pd.to_datetime(
                    manning_df["PLANNED_DATES"]
                )
                ...
                for date in sorted(unique_dates):
                    ...
                    emp_fact_df["remaining_capacity"] = emp_fact_df[
                        "average_capacity"
                    ].copy()
                    emp_fact_df["remaining_capacity"] = emp_fact_df[
                        "remaining_capacity"
                    ...
                    if (emp_fact_df["remaining_capacity"] < 0).any():
                        logger.info(
                            f"WARNING: Found negative initial capacities on {date}!"
                        )
                        # Fix any negative values by setting to 0
                        emp_fact_df.loc[
                            emp_fact_df["remaining_capacity"] < 0, "remaining_capacity"
                        ] = 0

                    # Filter orders for the current date
                    daily_orders = manning_df[
                        manning_df["PLANNED_DATES"] == date
                    ].copy()
                    ...
                    for index, row in daily_orders.iterrows():
                        ...
                        if available_employees.empty:
                            ...
                            any_matching_code = not emp_fact_df[
                                emp_fact_df["code"] == code
                            ].empty

                            if not any_matching_code:
                                original_row["SHORTAGE_REASON"] = (
                                ...
                                if other_lines:
                                    original_row["SHORTAGE_REASON"] = (
                                        f"CODE={code} found only in lines: {', '.join(set(other_lines))}"
                                    )
                                else:
                                    # Check if there are employees in this line but with zero capacity
                                    zero_capacity = not emp_fact_df[
                                        (emp_fact_df["code"] == code)
                                        & (emp_fact_df["line"] == line)
                                    ...
                                    if zero_capacity:
                                        ...
                                        original_row["SPLIT_ORDER_ID"] = (
                                            ""  # No split for unresolved shortages
                                        )
                        ...
                        while remaining_qty > 0 and not available_employees.empty:
                            ...
                            if new_capacity < 0:
                                logger.info(
                                    f"Fixing floating-point error: {new_capacity} set to 0"
                                )
                                new_capacity = 0
                            ...
                            if first_allocation:
                                ...
                                current_row["SPLIT_ORDER_ID"] = (
                                    f"{split_order_id}_part1"
                                )
                                current_row["PERIOD"] = df_info[
                                    "period"
                            ...
                            current_row["PLANNED_QTY"] = float(
                                allocation
                            )  # Update planned qty for the split PLANNED_QTY
                            ...
                            if available_employees.empty and remaining_qty > 0:
                                ...
                                shortage_row["PLANNED_QTY"] = float(
                                    remaining_qty
                                )  # PLANNED_QTY
                                ...
                                shortage_row["SPLIT_ORDER_ID"] = (
                                    f"{split_order_id}_shortage"
                                )
                                shortage_row["PERIOD"] = df_info[
                                    "period"
                                ]  # Ensure PERIOD is set
                                ...
                                if capacity_in_line == 0:
                                    shortage_row["SHORTAGE_REASON"] = (
                                        f"All employees with CODE={code} in LINE={line} are fully allocated"
                                    )
                                else:
                                    # Modified to prevent showing negative capacity
                                    shortage_row["SHORTAGE_REASON"] = (
                                        f"Insufficient capacity: Still needed {remaining_qty} units but only {capacity_in_line} available"
                                    )
                    ...
                    neg_capacity_emps = emp_fact_df[
                        emp_fact_df["remaining_capacity"] < 0
                    ]
                    if not neg_capacity_emps.empty:
                        logger.info(
                            f"ERROR: Found {len(neg_capacity_emps)} employees with negative capacity on {date}:"
                        )
                        for _, neg_emp in neg_capacity_emps.iterrows():
                            logger.info(
                                f"  - Employee {neg_emp['employee_id']}: {neg_emp['remaining_capacity']}"
                            )
                ...
                try:
                    updated_manning_df = updated_manning_df.merge(
                        df_unique_smv[["style", "smv"]],
                        left_on="STYLE",
                        right_on="style",
                        how="left",
                    )
                    # Ensure NaN values are replaced with None
                    updated_manning_df = updated_manning_df.where(
                        pd.notna(updated_manning_df), None
                    )
                    ...
                    data_list = [
                        ManningSheetData(
                            ...
                            if pd.notna(row["ALLOCATED EMP ID"])
                            else 0,  # Convert NaN to 0
                    ...
                    for i in range(0, len(data_list), CHUNK_SIZE):
                        with transaction.atomic():  # Ensure bulk insert is atomic
                            ManningSheetData.objects.bulk_create(
                    ...
                    return error_response(
                        error=f"SMV data error. Message: {str(e)}",
                ...
                return error_response(
                    error=f"Failed to generate manning sheet. Message: {str(e)}",
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
        ...
        for period, manning in manning_sheet_df.items():
            ...
            if "ALLOCATED CAPACITY" in df_manning.columns:
                ...
                df_manning["PLANNED_QTY"] = (
                    df_manning["PLANNED_QTY"].fillna(0).astype(int)
                )
                ...
                df_manning = df_manning.groupby(
                    ["STYLE", "LINE", "SECTION", "CODE", "MACHINE_TYPE"], as_index=False
            ...
            df_section_avg_capacity = df_emp_fact_filtered.groupby(
            ...
            df_total_active_operators = df_emp_fact_filtered.groupby(
                ["code", "line", "section"], as_index=False
            )["employee_id"].count()
            ...
            df_machinist_count = (
                df_emp_fact_filtered[df_emp_fact_filtered["designation"] == "Machinist"]
            ...
            df_merged_filtered = df_manning.merge(
                df_median_capacity,
                left_on=["CODE", "LINE", "SECTION"],
                right_on=["code", "line", "section"],
                how="left",
            )
            df_merged_filtered = df_merged_filtered.merge(
                df_section_avg_capacity, on=["section", "line"], how="left"
            )
            df_merged_filtered = df_merged_filtered.merge(
                df_total_active_operators, on=["code", "line", "section"], how="left"
            )
            df_merged_filtered = df_merged_filtered.merge(
                df_machinist_count, on=["code", "line", "section"], how="left"
            )
            df_merged_filtered = df_merged_filtered.merge(
                df_non_machinist_count, on=["code", "line", "section"], how="left"
            )

            # Fill missing capacity values
            df_merged_filtered["Median_Average_Capacity"].fillna(
                df_merged_filtered["Section_Average_Capacity"], inplace=True
            )
            df_merged_filtered["Median_Average_Capacity"].replace(
                0, 1, inplace=True
            ...
            df_merged_filtered["Total_Operators_Required"] = (
                df_merged_filtered["PLANNED_QTY"]
                / df_merged_filtered["Median_Average_Capacity"]
            ...
            df_merged_filtered["Machinist_Required"] = df_merged_filtered.apply(
                lambda row: (
                    round(row["Total_Operators_Required"], 1)
                    if row["code"]
                    in df_emp_fact_filtered[
                        df_emp_fact_filtered["designation"] == "Machinist"
                    ]["code"].values
                    else 0
                ),
                axis=1,
            ...
            machine_data = (
                df_emp_fact_filtered.groupby("code")["machine"].unique().reset_index()
            )
            ...
            df_merged_filtered = df_merged_filtered.merge(
                machine_data, on="code", how="left"
            )

            df_results.append(df_merged_filtered)

        df_final_Information = pd.concat(df_results, ignore_index=True)
        df_final_Information = df_final_Information.where(
            pd.notna(df_final_Information), None
        )
        ...
        return success_response(
            message="Successfully generated manning data", status=status.HTTP_200_OK
        )
...
def run_generate_emp_fact():
    try:
        ...
        df_skill_matrix["employeeId"] = (
            df_skill_matrix["employeeId"].astype(str).str.replace(r"\D", "", regex=True)
        )
        df_skill_matrix["employeeId"] = pd.to_numeric(
            df_skill_matrix["employeeId"], errors="coerce"
        )
        ...
        return success_response(
            message="Data processed and uploaded to Database", status=status.HTTP_200_OK
        )
    except Exception as e:
        logger.info(f"Error in run_generate_emp_fact: {str(e)}")
        logger.error(
            f"Error in run_generate_emp_fact: {str(e)} at {datetime.now()} hours!",
            exc_info=True,
        )
...
def run_manning_generation(viaAPI, PERIOD):
    try:
        ...
        if not isWorkingDay:
            return error_response(
                error=f"Skipping for {today} as it is {reason}",
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not viaAPI:
            ...
            logger.info(
                f"Running Manning Sheet generation at {str(current_time)} hours!"
            )
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_lp = executor.submit(
                lambda: list(LoadingPlan.objects.values().iterator())
            )
            future_style = executor.submit(
                lambda: list(StyleOB.objects.values().iterator())
            )
            future_empfact = executor.submit(
                lambda: list(EMPFact.objects.values().iterator())
            )
            future_active_employees = executor.submit(
                lambda: list(ActiveEmployees.objects.values().iterator())
            )
        ...
        # Read data from DB
        date_thresholds = {f"{PERIOD}_days": today + timedelta(days=PERIOD)}
        df_load_plan_transformed["planned_dates"] = pd.to_datetime(
            df_load_plan_transformed["planned_dates"], errors="coerce"
        )
        df_load_plan_transformed["planned_dates"] = df_load_plan_transformed[
        ...
        filtered_dfs = filter_by_date_ranges(
            df_load_plan_transformed, today, date_thresholds, PERIOD
        )
        ...
        for item in df_load_plan_transformed["line"].unique():
            ...
            df_load_plan_transformed_updated = df_load_plan_transformed[
                df_load_plan_transformed["line"] == item
            ]
        ...
        # Combine all collected DataFrames
        all_unallocated_employees = pd.concat(
            all_unallocated_employees, ignore_index=True
        )
        # Drop duplicates ignoring REASON, CATEGORY, and PERIOD
        all_unallocated_employees.drop_duplicates(
            subset=[
                col
                for col in all_unallocated_employees.columns
                if col not in ["REASON", "CATEGORY", "PERIOD"]
            ],
            inplace=True,
            ignore_index=True,
        )
...
def run_dday_generation(viaAPI):
    try:
        ...
        try:
            ...
            return error_response(
                error=f"Error in fetching and transforming attendance data: {str(e)}",
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            attendance_queryset = AttendanceMaster.objects.filter(
                attendance_date=allocation_date
            ).values()
            ...
            if emp_fact_df.empty or Act_Employees.empty:
                return error_response(
                    error="Cannot generate Manning Sheet because Employee Fact or Active Employees data is missing.",
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if attendance_df.empty:
                return error_response(
                    error="Cannot generate Manning Sheet because Attendance data for the specified date is missing.",
                    status=status.HTTP_400_BAD_REQUEST,
                )

            emp_fact_df = emp_fact_df[
                emp_fact_df["employee_id"].isin(Act_Employees["emp_code"])
            ]
            emp_fact_df = emp_fact_df[
                emp_fact_df["type"].isin(["Primary", "Secondary"])
            ]
        ...
        manning, stats, tracking = run_intraday_allocation_enhanced(
            allocation_date, attendance_df, emp_fact_df, wip_df, run_type=run_type
        )
        ...
        initial_completeness = (
            (initial_allocated / initial_planned * 100) if initial_planned > 0 else 0
        )
        ...
        attendance_df_temp["attendance_date"] = pd.to_datetime(
            attendance_df_temp["attendance_date"]
        )
        daily_attendance = attendance_df_temp[
            attendance_df_temp["attendance_date"] == allocation_date
        ]
        ...
        optimized_manning = perform_final_allocation_pass(
            manning, emp_fact_df, absent_set
        )

        # Calculate enhanced allocation completeness
        final_allocated = optimized_manning["allocated_capacity"].fillna(0).sum()
        final_completeness = (
            (final_allocated / initial_planned * 100) if initial_planned > 0 else 0
        )
        ...
        try:
            ...
            if "message" in gap_analysis:
                ...
                logger.info(
                    f"Operations with unmet quantities: {gap_analysis['unmet_operations_count']}"
                )
                logger.info(
                    f"Total unmet quantity: {gap_analysis['total_unmet_quantity']:.2f}"
                )
                logger.info(
                    f"Potential additional allocation: {gap_analysis['total_potential_additional_allocation']:.2f}"
                )
                ...
                if gap_analysis["potential_matches"]:
                    ...
                    logger.info("-" * 60)
                    for skill, data in list(gap_analysis["potential_matches"].items())[
                        :5
                    ]:
                        logger.info(f"Skill {skill}:")
                        logger.info(
                            f"  Unmet: {data['unmet_quantity']:.1f}, Available: {data['available_capacity']:.1f}"
                        )
                        logger.info(
                            f"  Potential fulfillment: {data['potential_fulfillment']:.1f} ({data['fulfillment_percentage']:.1f}%)"
                        )
                        logger.info()
        except Exception as e:
        ...
        if not unallocated_report.empty:
            unallocated_output_path = "exports/unallocated_report_dday.csv"
            unallocated_report.to_csv(unallocated_output_path, index=False)
            logger.info(
                f"Enhanced Unallocated report saved to: {unallocated_output_path}"
            )
        else:
            logger.info(
                "No unallocated employees found - all employees have been allocated!"
            )
        ...
        if not unallocated_report.empty:
            ...
            display_columns = [
                col for col in available_columns if col in unallocated_report.columns
            ]
        ...
        if not unallocated_report.empty:
            ...
            logger.info(
                f"Unique Unallocated Employees: {unallocated_report['employee_id'].nunique()}"
            )
            ...
            logger.info("\nUnallocated Employees Statistics:")
            logger.info("All employees have been allocated successfully!")

        # NEW: Show enhanced allocations made
        enhanced_allocations = optimized_manning[
            optimized_manning["reallocation_level"].isin(
                [
                    "final_optimization",
                    "final_optimization_addition",
                    "final_optimization_additional_row",
                ]
            )
        ]
        ...
        if not enhanced_allocations.empty:
            ...
            try:
                ...
                if len(common_indices) > 0:
                    original_capacity = (
                        manning.loc[common_indices, "allocated_capacity"]
                        .fillna(0)
                        .sum()
                    )
                    enhanced_capacity = (
                        enhanced_allocations.loc[common_indices, "allocated_capacity"]
                        .fillna(0)
                        .sum()
                    )
                    ...
                    additional_capacity = (
                        enhanced_allocations["allocated_capacity"].fillna(0).sum()
                    )

                logger.info(f"Additional capacity allocated: {additional_capacity:.2f}")
            except Exception:
                logger.info(
                    f"Additional capacity calculation: {enhanced_allocations['allocated_capacity'].fillna(0).sum():.2f}"
                )
            ...
            available_cols = [
                col for col in display_cols if col in enhanced_allocations.columns
            ]
            ...
            logger.info("\n" + "=" * 60)
            logger.info(
                "No additional allocations were made - your original allocation was already optimal!"
            )
            logger.info("=" * 60)
        ...
        logger.info("=" * 80)
        logger.info(
            f"🎯 Allocation Improvement: {initial_completeness:.2f}% → {final_completeness:.2f}% (+{improvement:.2f}%)"
        )
        logger.info(f"📊 Total Planned Quantity: {initial_planned:.2f}")
        logger.info(
            f"📈 Additional Capacity Allocated: {final_allocated - initial_allocated:.2f}"
        )
        logger.info(
            f"👥 Enhanced Allocations Made: {len(enhanced_allocations) if not enhanced_allocations.empty else 0}"
        )
        if not unallocated_report.empty:
            logger.info(
                f"❌ Unallocated Employees: {unallocated_report['employee_id'].nunique()}"
            )
        else:
            logger.info("✅ All Employees Allocated Successfully!")
        logger.info("=" * 80)
        ...
        try:
            if not manning_records:
                return success_response(
                    message="No records generated for DDay", status=status.HTTP_200_OK
                )
            ...
            if len(manning_records) > 0:
                for i in range(0, len(manning_records), CHUNK_SIZE):
                    ...
                    with transaction.atomic():
                        DDayData.objects.bulk_create(
                            manning_data, ignore_conflicts=True
                        )
        ...
        if not viaAPI:
            ...
            logger.info("***************************************************\n\n")
        return success_response(
            message="Successfully generated DDay data", status=status.HTTP_200_OK
        )
...
def run_generate_style_ob(viaAPI):
    try:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            data_dicts = list(
                executor.map(row_to_dict, [row for _, row in df_Style_OB.iterrows()])
            )
        ...
        logger.error(
            f"Error in run_generate_style_ob: {str(e)} at {datetime.now()} hours!",
            exc_info=True,
        )
        return error_response(error=str(e), status=status.HTTP_400_BAD_REQUEST)
