...
def redistribute_production_plan(
    data, line_capacities=None, respect_date_ranges=True, max_styles_per_day=None
):
    """
    ...
    if "FABRIC ARTICLE" not in data.columns:
        logger.info(
            "Warning: 'FABRIC ARTICLE' column not found in data. Adding default value."
        )
        data["FABRIC ARTICLE"] = (
            "DEFAULT"  # Add a default fabric article if the column doesn't exist
        )
    ...
    if line_capacities is None:
        ...
        for line in lines:
            if line not in line_capacities:
                ...
                logger.info(
                    f"Warning: No capacity defined for {line}, using default 1300"
                )
    ...
    for line in lines:
        logger.info(
            f"Processing {line} (Capacity: {line_capacities[line]} units/day)..."
        )
        ...
        for week, week_df in line_data.groupby("Week"):
            ...
            logger.info(
                f"  Week {week}: {len(style_fabric_order)} style+fabric combinations in sequence"
            )
        ...
        if has_order_identifiers:
            ...
            for idx, row in line_data.iterrows():
                ...
                if (
                    week in style_fabric_week_sequence
                    and style_fabric_key in style_fabric_week_sequence[week]
                ):
                    sequence = style_fabric_week_sequence[week][style_fabric_key]
                    first_appearance_date = (
                        del_date  # style_fabric_week_first_date[week][style_fabric_key]
                    )
                else:
                    sequence = 999
            ...
            orders.sort(
                key=lambda x: (x["week"], x["priority"], x["sequence"], -x["qty"])
            )

        else:
            groups = []
            for (style, fabric_article, week, del_date), group_df in line_data.groupby(
                ["STYLE", "FABRIC ARTICLE", "Week", "DEL DATE"]
            ):
        ...
        day_styles_fabrics = {
            date: set() for date in planned_dates
        }  # Track style+fabric combinations
        style_fabric_allocation = defaultdict(
            list
        )  # Track where each style+fabric is allocated

        # Add max styles tracking
        if max_styles_per_day:
            logger.info(
                f"Limiting {line} to maximum {max_styles_per_day} style+fabric combinations per day"
            )
        ...
        for order in orders:
            ...
            if (
                "Date_Start" in order["template_row"]
                and "Date_End" in order["template_row"]
            ):
                date_start = pd.to_datetime(order["template_row"]["Date_Start"])
                date_end = pd.to_datetime(order["template_row"]["Date_End"])
                ...
                valid_week_dates = [
                    date for date in week_dates if date_start <= date <= date_end
                ]
            ...
            for date in valid_week_dates:
                ...
                if (
                    max_styles_per_day is not None
                    and style_fabric_key not in day_styles_fabrics[date]
                    and len(day_styles_fabrics[date]) >= max_styles_per_day
                ):
                    continue
            ...
            if remaining_qty > 0:
                adjacent_weeks = [
                    w for w in week_to_dates.keys() if abs(w - week) <= 1 and w != week
                ]
                adjacent_dates = []
                ...
                split_count = len(
                    [
                        ...
                        for r in new_plan
                        if (
                            has_order_identifiers
                            and "Split_ID" in r
                            and r["Split_ID"].startswith(f"{order['row_idx']}_")
                        )
                    ]
                )
            ...
            if remaining_qty > 0:
                available_days = [
                    (date, daily_capacity - day_loads[date])
                    for date in planned_dates
                ...
                if (
                    ...
                    available_days = [
                        (date, capacity)
                        for date, capacity in available_days
            ...
            if remaining_qty > 0:
                logger.info(
                    f"Warning: Could not fully allocate {style} (fabric article: {fabric_article}) within capacity limits."
                )
                logger.info(
                    f"         Allowing slight overloading to accommodate {remaining_qty} units"
                )
                ...
                style_fabric_days = [
                    date
                    for date in planned_dates
                    if style_fabric_key in day_styles_fabrics[date]
                ]
                ...
                if (
                    ...
                    date_start = pd.to_datetime(order["template_row"]["Date_Start"])
                    date_end = pd.to_datetime(order["template_row"]["Date_End"])
                    filtered_style_fabric_days = [
                        date
                        for date in style_fabric_days
                        if date_start <= date <= date_end
                    ]
                ...
                if style_fabric_days:
                    ...
                    logger.info(
                        f"         Added to existing style+fabric day {best_day}, new load: {day_loads[best_day]}"
                    )
                    remaining_qty = 0
                else:
                    ...
                    if same_week_days:
                        ...
                        logger.info(
                            f"         Added to same week day {best_day}, new load: {day_loads[best_day]}"
                        )
                        remaining_qty = 0
                    else:
                        ...
                        if (
                            ...
                            date_start = pd.to_datetime(
                                order["template_row"]["Date_Start"]
                            )
                            date_end = pd.to_datetime(order["template_row"]["Date_End"])
                            filtered_days = [
                                date
                                for date in same_week_days
                                if date_start <= date <= date_end
                            ]
                        ...
                        if not valid_days:
                            ...
                            logger.info(
                                f"         WARNING: No valid days within date constraints for {style}, using any available day"
                            )

                        # Find the day with the lowest load
                        best_day = (
                            min(valid_days, key=lambda d: day_loads[d])
                            if valid_days
                            else planned_dates[0]
                        )
                        ...
                        if not best_day:
                            ...
                            logger.info(
                                f"         CRITICAL: Using first available date as fallback for {style}"
                            )
                        ...
                        day_styles_fabrics[best_day].add(style_fabric_key)
                        logger.info(
                            f"         Added to day {best_day} as last resort, new load: {day_loads[best_day]}"
                        )
                        remaining_qty = 0
...
def analyze_redistribution(original_df, new_df, line_capacities):
    ...
    logger.info(
        f"Original plan: {original_overloaded} days exceeding capacity ({original_total_over:.2f} total units over)"
    )
    logger.info(
        f"New plan: {new_overloaded} days exceeding capacity ({new_total_over:.2f} total units over)"
    )

    # Analyze style distribution
    original_style_fabric_count = [
        len(style_fabrics) for style_fabrics in original_style_fabrics.values()
    ]
    new_style_fabric_count = [
        len(style_fabrics) for style_fabrics in new_style_fabrics.values()
    ]

    original_avg_style_fabrics = (
        sum(original_style_fabric_count) / len(original_style_fabric_count)
        if original_style_fabric_count
        else 0
    )
    new_avg_style_fabrics = (
        sum(new_style_fabric_count) / len(new_style_fabric_count)
        if new_style_fabric_count
        else 0
    )

    original_single_style_fabric = sum(
        1 for count in original_style_fabric_count if count == 1
    )
    new_single_style_fabric = sum(1 for count in new_style_fabric_count if count == 1)

    logger.info(
        f"Original plan: {original_avg_style_fabrics:.2f} avg style+fabric combinations per day, {original_single_style_fabric} single-combination days"
    )
    logger.info(
        f"New plan: {new_avg_style_fabrics:.2f} avg style+fabric combinations per day, {new_single_style_fabric} single-combination days"
    )
    ...
    for line in lines:
        ...
        original_line_util = [
            original_loads.get(key, 0) / capacity * 100 for key in line_keys
        ]
        ...
        if original_line_util:
            ...
            logger.info(
                f"{line}: Original capacity utilization: {orig_avg:.2f}%, New: {new_avg:.2f}%"
            )
...
def order_split_analysis(new_df):
    ...
    avg_split_count = (
        sum(split_counts.values()) / len(split_counts) if split_counts else 0
    )

    logger.info("\nOrder split analysis:")
    logger.info(f"  Total orders: {total_orders}")
    logger.info(
        f"  Orders split across multiple days: {total_split_orders} ({total_split_orders / total_orders * 100:.1f}%)"
    )
