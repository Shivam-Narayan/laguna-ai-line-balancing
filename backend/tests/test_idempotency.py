...
def test_idempotency_first_request():
...
def test_idempotency_duplicate_request():
    ...
    assert data2["run_count"] == 1
    
    # The request2 object itself should not have been mutated
    assert request2.run_count == 0

def test_idempotency_missing_key():
    factory = APIRequestFactory()
    ...
    data2 = json.loads(response2.content)
    assert data2["run_count"] == 2 # View ran again because no key was provided
